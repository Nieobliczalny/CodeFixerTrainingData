#include <cstdio>
#include <cstdint>
#include <cstdlib>
#include <iostream>
#include <string>
#include <cmath>


using namespace std;

long  RU09(unsigned long long  HYPw1,long  PGhbj9H);

short  v46xWeT8(unsigned short int  OYrn2);

unsigned short    WHC3m1Q2rt(short int    w_7c48E7f);

int main() {
    long long    nPUWL3;
    nPUWL3    =    0;
  unsigned long long  ya_90;
  ya_90  =  2006717631;
 int HJU9;
 HJU9 = 0;
  unsigned short int  W9k7cT879;
  W9k7cT879  =  1500940194;
    unsigned short int    ZMhaQ2;
    ZMhaQ2    =    -1402053583;
  unsigned short  q82Oi;
  q82Oi  =  750620336;
 short int Xa_gb1F;
 Xa_gb1F = 1192638379;
    short int    oU44;
    oU44    =    1002659432;
	long double	pQ_n;
	pQ_n	=	-1638664703;
    unsigned short    zfb1FX;
    zfb1FX    =    -199373355;
 char lEYL;
 lEYL = -1439136969;
  int  Dw6qlz2;
  Dw6qlz2  =  -299667621;
  unsigned long  TU1Ck;
  TU1Ck  =  0;
  unsigned int  JLmiA_uqGc;
  JLmiA_uqGc  =  589554731;

if (lEYL == 547270676)
{
{
int j = 1;
do
{
if (zfb1FX > 1003799150)
{
HJU9 = WHC3m1Q2rt(111314696);
}
else if ((oU44 == 855602685) && ((W9k7cT879 != pQ_n) && (q82Oi <= Dw6qlz2)))
{
lEYL ^= 1915010900;
}
else if (ya_90 >= 1454214247)
{
zfb1FX = v46xWeT8(204105718);
}
else
{
lEYL <<= 377942456;
}
j--;
}
while (j >= 0);
}
}
switch (TU1Ck)
{
case 'a':
HJU9 = WHC3m1Q2rt(W9k7cT879);
break;
case 'b':
pQ_n += 1549036021;
break;
case 'c':
nPUWL3 = WHC3m1Q2rt(Dw6qlz2);
break;
case 'd':
oU44 = RU09(Dw6qlz2, 656678099) ^ Dw6qlz2;
break;
case 'e':
pQ_n = (int)abs(nPUWL3) + (-337146371);
break;
case 'f':
zfb1FX &= 852321851;
break;
case 'g':
pQ_n = Xa_gb1F << 1420228191;
break;
case 'h':
q82Oi /= pQ_n;
break;
}
try
{
{
int j = 0;
do
{
j++;
{
int k = 0;
do
{
++k;
q82Oi ^= zfb1FX;
}
while (k < 10);
}
}
while (j < 6);
}
}
catch (int& errorCode)
{
Xa_gb1F = (int)abs(oU44);
}
Dw6qlz2 = (int)(nPUWL3 + (1980279167));
pQ_n = HJU9 % 223584421;

std::cout << "c_N_MovWqN" << "fu27" << "kWQ0_Y" << oU44 << std::endl;
HJU9 = (int)abs(pQ_n) * 0;
std::cout << WHC3m1Q2rt(Dw6qlz2) << oU44 << TU1Ck << ya_90 << RU09(-1410540422, -603180107) << std::endl;
TU1Ck = (unsigned long)(Dw6qlz2 * TU1Ck);
switch (lEYL)
{
case 0:
oU44 = (int)abs(nPUWL3) + (-1988829495);
break;
case 1:
Dw6qlz2 = RU09(1296471027, ya_90);
break;
case 2:
Xa_gb1F = RU09(-151530982, TU1Ck);
break;
case 3:
ya_90 = Dw6qlz2 | 1417629500;
break;
case 4:
JLmiA_uqGc = lEYL % 1561203348;
break;
case 5:
zfb1FX %= oU44;
break;
case 6:
pQ_n = (int)abs(nPUWL3) + (877753545);
break;
case 7:
oU44 -= HJU9;
break;
case 8:
oU44 = RU09(Xa_gb1F, 1115899473) >> JLmiA_uqGc;
break;
case 9:
ya_90 = v46xWeT8(ya_90);
break;
default:
lEYL = (int)abs(Dw6qlz2);
break;
}
if (((nPUWL3 > Dw6qlz2) && (Dw6qlz2 == 750649911)) && ((JLmiA_uqGc == pQ_n) && (ZMhaQ2 != 390242029)))
{
if (JLmiA_uqGc != Xa_gb1F)
{
ya_90 = RU09(-2008263026, 1657663001);
}
else
{
try
{
pQ_n = (int)abs(lEYL);
}
catch (int& errorCode)
{
lEYL = v46xWeT8(1484475306);
}
}
}
else
{
std::cout << "lmwf4t1" << std::endl;
}

if (Xa_gb1F & -1237017418)
{
if (q82Oi == -717945217)
{
nPUWL3 = (long long)(q82Oi * nPUWL3);
}
}
else
{
{
int j = 8;
while (j >= 0)
{
if (Xa_gb1F == 1036213747)
{
JLmiA_uqGc = v46xWeT8(1413063482);
}
else if ((ya_90 < -1942780396) && (TU1Ck & -401894874))
{
Dw6qlz2++;
}
else if (Dw6qlz2)
{
HJU9 = WHC3m1Q2rt(775338238);
}
else
{
lEYL = RU09(-299117220, ZMhaQ2);
}
--j;
}
}
}
lEYL = RU09(lEYL, -410209885);
{
int i = 5;
while (i >= 0)
{
--i;
JLmiA_uqGc ^= ya_90;
}
}
oU44 = q82Oi & ya_90;

Dw6qlz2 = (int)abs(HJU9) + (-592504080);

switch (ZMhaQ2)
{
case 'a':
case 'A':
HJU9 = WHC3m1Q2rt(JLmiA_uqGc);
break;
case 'b':
case 'B':
zfb1FX = v46xWeT8(418087638);
break;
case 'c':
case 'C':
TU1Ck = WHC3m1Q2rt(686088364);
break;
case 'd':
case 'D':
pQ_n = RU09(-1495158242, 611832906);
break;
default:
q82Oi = Dw6qlz2 * 875406616;
break;
}
std::cout << "Llc5" << "oJ8F1" << "bb5_E" << std::endl;

if (WHC3m1Q2rt(-264246698) == 0)
{
	Dw6qlz2 = RU09(591784678, -518344656) / WHC3m1Q2rt(-264246698);
}

JLmiA_uqGc = RU09(pQ_n, zfb1FX) | 1414984191;

ZMhaQ2 ^= 1353478958;
HJU9 = (int)(Xa_gb1F * HJU9);

switch (JLmiA_uqGc)
{
case 0:
q82Oi ^= 904824631;
break;
case 1:
pQ_n = oU44 | 1558529815;
break;
default:
nPUWL3 = WHC3m1Q2rt(-1000958892);
break;
}
return 0;
}

long  RU09(unsigned long long  HYPw1,long  PGhbj9H)
{
  return  538606760;
}

short  v46xWeT8(unsigned short int  OYrn2)
{
  return  -411156679;
}

unsigned short    WHC3m1Q2rt(short int    w_7c48E7f)
{
    return    0;
}

