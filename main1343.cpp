#include <cstdlib>
#include <iostream>
#include <string>
#include <cstdint>
#include <cstdio>
#include <cmath>

#define	YHaS8D	(1130432917)
#define	TkYI0a7_	(0)
#define  kXT_  (789934886)

using namespace std;

unsigned short int X3o8CKN();

unsigned short int	QvqLE7E_R7(int	yN5_w9,int	rUIJB,char	DIPEvL,unsigned long	Zv3X4u98V4,char	IMDd);

short  aU0tt(unsigned short  kp7vMfgE,unsigned short  FwZ6o12g,unsigned short int  CrURpv26i5,char  P_7b,unsigned short int  vdPeZCO6Mz);

int main(void) {
    unsigned int    N5no,    E5IiDEjE;
    N5no    =    0;
    E5IiDEjE    =    0;
  long double  r_y_fns8,  us_9w,  DEOaUN,  een00B57,  k6Ql13QJT,  z827MQ,  c48Z,  yDlMan,  a1gPE,  skxA,  hCBnx,  cTT_6,  c72SEjw,  J3xx1PPzU5,  j2sF4v,  do7r,  C26MaUdmz,  nKB_6,  I4AQCz,  K7AmKGeV,  DR4XM3lWi,  N1w5,  A999,  AFL6NYhVG,  BLH1L6T0,  Gbl4,  a_8Y9NUbqw,  l6s1,  h45Gts,  AAR87,  CUDhF3Lh,  Tqg5Ab4C19,  weHI7,  Jv1_q_pgE6,  Yi0N4,  yI43KJ___,  sylP,  hSqMFIa0,  ygr9gEZ,  ee79j5q,  ATpH4uo,  Y8v55Z0O,  enuiiiT5TP,  ejT87tC,  W7Y04M,  SF4e,  c0CwACwl5,  B11l38,  aDiw,  erlV,  a72j4,  Fn6a8,  JbjUsM,  h6A4d7oO,  EV_378iz,  u_jVkx7Xo,  Ek8F6hMq;
  r_y_fns8  =  -62513946;
  us_9w  =  2143991162;
  DEOaUN  =  -588274961;
  een00B57  =  1062242521;
  k6Ql13QJT  =  -1163750531;
  z827MQ  =  -1665680750;
  c48Z  =  922597381;
  yDlMan  =  1329143204;
  a1gPE  =  1467182289;
  skxA  =  431660519;
  hCBnx  =  -604223954;
  cTT_6  =  2143517183;
  c72SEjw  =  1107363162;
  J3xx1PPzU5  =  1212703001;
  j2sF4v  =  -248598403;
  do7r  =  743279348;
  C26MaUdmz  =  -1540740028;
  nKB_6  =  -148229246;
  I4AQCz  =  40271722;
  K7AmKGeV  =  -1625715539;
  DR4XM3lWi  =  1059988045;
  N1w5  =  -546304702;
  A999  =  -1962862729;
  AFL6NYhVG  =  -107818702;
  BLH1L6T0  =  -1547873054;
  Gbl4  =  -1438129126;
  a_8Y9NUbqw  =  777092533;
  l6s1  =  -569215812;
  h45Gts  =  1008792901;
  AAR87  =  427025802;
  CUDhF3Lh  =  6137363;
  Tqg5Ab4C19  =  1386719211;
  weHI7  =  -486712502;
  Jv1_q_pgE6  =  -781476060;
  Yi0N4  =  384626861;
  yI43KJ___  =  -1116964744;
  sylP  =  -397815822;
  hSqMFIa0  =  1561297493;
  ygr9gEZ  =  -2135950576;
  ee79j5q  =  -114526626;
  ATpH4uo  =  -703136792;
  Y8v55Z0O  =  1337597461;
  enuiiiT5TP  =  -971647438;
  ejT87tC  =  -1858460075;
  W7Y04M  =  279048907;
  SF4e  =  460764337;
  c0CwACwl5  =  1861005214;
  B11l38  =  -1208557807;
  aDiw  =  -2023889549;
  erlV  =  1183137444;
  a72j4  =  -474190227;
  Fn6a8  =  961564298;
  JbjUsM  =  -1701343025;
  h6A4d7oO  =  702029937;
  EV_378iz  =  932610056;
  u_jVkx7Xo  =  -1645634947;
  Ek8F6hMq  =  -1753588031;

std::cout << "b9_kzH" << std::endl;
c72SEjw = JbjUsM / E5IiDEjE;

yDlMan = (int)floor(DEOaUN);
hSqMFIa0 += sylP;
try
{
c48Z = (int)abs(c0CwACwl5);
}
catch (int& errorCode)
{
EV_378iz = (int)floor(K7AmKGeV);
}

for (int i = 6; i >= 0; i--)
{
{
int j = 0;
while (j < 2)
{
if (((N5no & TkYI0a7_) && (C26MaUdmz <= AFL6NYhVG)) || ((E5IiDEjE & N5no) || (K7AmKGeV >= kXT_)))
{
u_jVkx7Xo = aU0tt((unsigned short)-1682338943, (unsigned short)823993567, A999, Gbl4, (unsigned short)-1072745420);
}
else
{
Jv1_q_pgE6 = aU0tt((unsigned short)150341960, (unsigned short)1842635161, cTT_6, (char)567192919, (unsigned short)759281754);
}
j++;
}
}
}

switch (N5no)
{
case 'a':
skxA = (c48Z == Gbl4) ? Jv1_q_pgE6 : 342896653;
break;
case 'b':
cTT_6 = (do7r) ? do7r : DEOaUN;
break;
default:
K7AmKGeV = (N5no > -165344593) ? Ek8F6hMq : 977011227;
break;
}
for (int i = 4; i >= 0; --i)
{
for (int j = 5; j >= 0; j--)
{
do7r = (int)abs(J3xx1PPzU5);
}
}
return 0;
}

unsigned short int X3o8CKN()
{
 return 0;
}

unsigned short int	QvqLE7E_R7(int	yN5_w9,int	rUIJB,char	DIPEvL,unsigned long	Zv3X4u98V4,char	IMDd)	{
	return	(unsigned short)-160348945;
}

short  aU0tt(unsigned short  kp7vMfgE,unsigned short  FwZ6o12g,unsigned short int  CrURpv26i5,char  P_7b,unsigned short int  vdPeZCO6Mz)
{
  return  (short)-1592704764;
}

