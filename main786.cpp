#include <cstdio>
#include <cmath>
#include <cstdlib>
#include <string>
#include <cstdint>
#include <iostream>



char By54dT(long Jm4h,unsigned int T4lHVQ);

char P1H6V3HU0();

unsigned char    hhjF(unsigned long long    M8CI,char    oP5dU,unsigned short int    A68VHpiYUq,unsigned long long    MCbX9f);

unsigned char d1ym1lO(short int KbrzTL_,short ACwO1XW09);

int	I26mSI_(unsigned char	JX79,char	V3T_6EJ);

int main(int argc, char** argv)
{
	long double	Bglhb;
	Bglhb	=	583435892;
 char R52s_;
 R52s_ = (char)2000431085;
    unsigned short int    y422f4;
    y422f4    =    (unsigned short)-1757156223;
 float uRepUV8baZ;
 uRepUV8baZ = 846540323;
 unsigned long long Hw2T;
 Hw2T = -564693992;
 long double Q9Ot4r;
 Q9Ot4r = 644332905;
  double  y9650vCOG;
  y9650vCOG  =  -875353801;
	unsigned short	l34Q387r_;
	l34Q387r_	=	(unsigned short)-1795278808;
 short int Srt1_67MB;
 Srt1_67MB = 0;
 unsigned short int f4927l_1e;
 f4927l_1e = (unsigned short)-371121455;
    unsigned long long    hL19mq;
    hL19mq    =    749346776;
 unsigned char L_sn;
 L_sn = (unsigned char)104809620;
    long long    T92lVy4xUJ;
    T92lVy4xUJ    =    -1885350807;
  long long  eo8dakXG;
  eo8dakXG  =  -209672313;
 unsigned int igGQM;
 igGQM = -563307100;
	unsigned short	Lg08349ig0;
	Lg08349ig0	=	(unsigned short)-1984484045;
 unsigned char EK000e3Ok;
 EK000e3Ok = (unsigned char)1373703544;
    unsigned long    NQ94si03O_;
    NQ94si03O_    =    0;
 int OjC_C__5C;
 OjC_C__5C = -2125127448;
	unsigned long long	u4VY60;
	u4VY60	=	1659014241;
    long double    pc1p_L3;
    pc1p_L3    =    553039809;
    double    n7Km4GKOQk;
    n7Km4GKOQk    =    -1843012110;
  long long  GTv4Y06;
  GTv4Y06  =  -1649990417;
  unsigned long  diuze;
  diuze  =  -122880611;
 long zslI7jwil;
 zslI7jwil = 63644813;
	short	J61JTu2;
	J61JTu2	=	(short)-1495597261;
  long long  miJC1x1;
  miJC1x1  =  1580351898;
 char sft35Rz0l5;
 sft35Rz0l5 = (char)-854796631;
  unsigned int  eH_7y938;
  eH_7y938  =  -2090289784;
 float AkQSrw;
 AkQSrw = 1895796556;
  unsigned char  r3VNxAZpK1;
  r3VNxAZpK1  =  (unsigned char)1891525973;
 long double y0mlWC_;
 y0mlWC_ = -1903513703;
 unsigned short t3H1TwCZ0I;
 t3H1TwCZ0I = (unsigned short)-68436572;
  int  g2F7;
  g2F7  =  453252233;
  unsigned long  C7rEi2sbYT;
  C7rEi2sbYT  =  401100871;
	unsigned char	h0CJU4fT;
	h0CJU4fT	=	(unsigned char)-1892967289;
	unsigned long	b7rf;
	b7rf	=	-604887068;
	unsigned long	zpO05H;
	zpO05H	=	1227283315;
	long double	x3T88;
	x3T88	=	-423352310;
    int    TZAHGjEa;
    TZAHGjEa    =    2049651773;
  long double  Y42R0O33Q;
  Y42R0O33Q  =  -1911926402;
    char    t1EIXtFX;
    t1EIXtFX    =    (char)-1153499032;
    double    md7qXi6c;
    md7qXi6c    =    -581113151;
  char  V9_14lOd;
  V9_14lOd  =  0;
	unsigned int	lkm552;
	lkm552	=	827912108;
    long    ZTqJh_G;
    ZTqJh_G    =    2069280887;
 unsigned short N1NjtrBlYs;
 N1NjtrBlYs = (unsigned short)-678166476;
  short int  TYCO;
  TYCO  =  (short)-1932477524;
  float  MmWBn37;
  MmWBn37  =  -455279170;
  unsigned char  yKt5iQd;
  yKt5iQd  =  0;
  unsigned short int  u_dw7hYAqT;
  u_dw7hYAqT  =  (unsigned short)306854145;
	long	Sk3GO8_P79;
	Sk3GO8_P79	=	-1807256279;
    unsigned long    F6_B55;
    F6_B55    =    -1081238436;
 float RlCy;
 RlCy = 771707046;
	short int	Y9S9v;
	Y9S9v	=	(short)1995462140;
	long	q9q9ax_0g;
	q9q9ax_0g	=	-72090973;

if (Srt1_67MB == 0)
{
	b7rf = hhjF(781340900, (char)539030402, Q9Ot4r, 1347829623) / Srt1_67MB;
}
if (TZAHGjEa != -2015124051)
{
if ((x3T88 > diuze) || (MmWBn37 != 485853621))
{
y0mlWC_ = (int)abs(n7Km4GKOQk);
}
else
{
try
{
J61JTu2 = d1ym1lO((short)-850576660, g2F7) % t1EIXtFX;
}
catch (int& errorCode)
{
t3H1TwCZ0I <<= eo8dakXG;
}
}
}
else
{
for (int j = 10; j >= 0; j--)
{
if (u4VY60 != Lg08349ig0)
{
Q9Ot4r = (int)abs(y9650vCOG);
}
else
{
r3VNxAZpK1 = I26mSI_(V9_14lOd, (char)-889145805);
}
}
}
u_dw7hYAqT = (unsigned short int)(T92lVy4xUJ);
ZTqJh_G = d1ym1lO((short)-1961545019, r3VNxAZpK1) & L_sn;

return 0;
}

char By54dT(long Jm4h,unsigned int T4lHVQ)
{
 return 0;
}

char P1H6V3HU0() {
 return (char)-605198070;
}

unsigned char    hhjF(unsigned long long    M8CI,char    oP5dU,unsigned short int    A68VHpiYUq,unsigned long long    MCbX9f)    {
    return    (unsigned char)932733945;
}

unsigned char d1ym1lO(short int KbrzTL_,short ACwO1XW09) {
 return (unsigned char)-1431334828;
}

int	I26mSI_(unsigned char	JX79,char	V3T_6EJ)	{
	return	-706441755;
}

