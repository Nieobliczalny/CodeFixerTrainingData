#include <cstdio>
#include <cmath>
#include <cstdlib>
#include <iostream>
#include <cstdint>
#include <string>


using std::cout;
using std::endl;
using std::abs;
using std::ceil;
using std::floor;

char	gY1_(unsigned short	gm_t,unsigned char	CJp2);

long long    F_HQFry9(int    GpJ6Mbijnr,char    ia0FO3q23,unsigned short    j9M6qQIWt,unsigned short int    pTPP4_30_,long long    FL7u9_SK);

unsigned int    aj6A(unsigned char    RbK8_u,short int    FIHy6a1o,short    vVN0E);

int main(int argc, char** argv)
{
    short int    jf8Pu5zT,    O52uZX,    DHWRW2mI,    fdiH;
    jf8Pu5zT    =    0;
    O52uZX    =    0;
    DHWRW2mI    =    0;
    fdiH    =    0;
 unsigned long dJ1E9V, os5nn, cv07_, tEB1n, oKhlsrXz9J, qI4Xz, kMNgSD5We, a2N1, GJJ5PEi96, r3iIK, G49nHFmsaL, IB7Z0n76BL, Xf3LpiD2, vZXzbC2D, ZuHp00x, V871Gk, QW82e1I, vEbr2Vrl, ZriOV, P7Gn, I50x_Hx3O, mr3x8M784y, Lc1jC, dmncJLr9, zi1m_T, nqyt6y, xj59_996, ec72z7JE, P6n28, QLw62octA4, bcXu, h5ol7a, u3kKMUEATm, PulDRKK, SCTiA2Zl, p2q_KG, GwO72RW6, B33oJ818re, x7CzHcm, C4nPRR, SFDI, TrK3F6R, hGfG24it, eTlVC_3N, V4sUB83711, QG3F5, nHcm, MVf825N, dw8U7suc, r_bn76, B_BPgY, UFou3GV9y2, z8q40MbrX, DZ69k2e, E9gTW24__, KJWZ5IO, w7N4rT, YrKZK, I_eRRV, RCb_4Nh4, s4_8mq, AdR6, l4cfI07, wJsg, Zi41E6, Wy9cOAtyf, Bt5DxGr9L, a5rD7pP, kf8I, s0mggB, p8nGc34, Mn8pi9MOe2, y6a9rP, Ckrz2l4sL, IBW61t, sv2wt_oKxs, H2K_12_, a3y_u, Am6e40Mx45, mMGa5Qf, Hje1_3, GTsBgT, A3Nc6_Irw, QgX_, uMfHM6Z, ifigu3, a7dh_0, qQ98U1J, XZiBXO68, L3l9sKo;
 dJ1E9V = -356183346;
 os5nn = 990518140;
 cv07_ = -154332233;
 tEB1n = -1824072389;
 oKhlsrXz9J = -1450142869;
 qI4Xz = 410441831;
 kMNgSD5We = 1956362755;
 a2N1 = 1025118556;
 GJJ5PEi96 = -336091839;
 r3iIK = -1139413325;
 G49nHFmsaL = 971295717;
 IB7Z0n76BL = 210597551;
 Xf3LpiD2 = 1601447141;
 vZXzbC2D = -532784959;
 ZuHp00x = 1019908105;
 V871Gk = 1858450344;
 QW82e1I = 820298239;
 vEbr2Vrl = -648539135;
 ZriOV = -148100156;
 P7Gn = 713950711;
 I50x_Hx3O = 1742687411;
 mr3x8M784y = -1450813281;
 Lc1jC = -472954183;
 dmncJLr9 = 116883966;
 zi1m_T = -900872895;
 nqyt6y = 980582637;
 xj59_996 = -1825312502;
 ec72z7JE = 1719373389;
 P6n28 = -1208680885;
 QLw62octA4 = -398267513;
 bcXu = -69281834;
 h5ol7a = -1288550914;
 u3kKMUEATm = 311305564;
 PulDRKK = 526811088;
 SCTiA2Zl = 434141078;
 p2q_KG = 525514518;
 GwO72RW6 = 1065238478;
 B33oJ818re = -58130183;
 x7CzHcm = -392350422;
 C4nPRR = 2060890686;
 SFDI = -557154366;
 TrK3F6R = -1579691709;
 hGfG24it = -1611736075;
 eTlVC_3N = -1298966086;
 V4sUB83711 = -1749223556;
 QG3F5 = 1045901495;
 nHcm = -1230391483;
 MVf825N = -726094935;
 dw8U7suc = -673276494;
 r_bn76 = -1566976143;
 B_BPgY = 1706594814;
 UFou3GV9y2 = 1144935851;
 z8q40MbrX = -210649459;
 DZ69k2e = 1141211364;
 E9gTW24__ = -2121495570;
 KJWZ5IO = 1963597500;
 w7N4rT = 326968226;
 YrKZK = 399899283;
 I_eRRV = 149394058;
 RCb_4Nh4 = -2099867101;
 s4_8mq = -1977589085;
 AdR6 = -1243029165;
 l4cfI07 = 974653625;
 wJsg = 1545271407;
 Zi41E6 = 268033749;
 Wy9cOAtyf = -2098708224;
 Bt5DxGr9L = 1054530497;
 a5rD7pP = 584169746;
 kf8I = 1027846002;
 s0mggB = -428062870;
 p8nGc34 = -491376968;
 Mn8pi9MOe2 = 205760385;
 y6a9rP = 1853066937;
 Ckrz2l4sL = -2071855073;
 IBW61t = 543698020;
 sv2wt_oKxs = 524969597;
 H2K_12_ = 402721796;
 a3y_u = 1860406231;
 Am6e40Mx45 = -1283134676;
 mMGa5Qf = -833868191;
 Hje1_3 = -1223414648;
 GTsBgT = -1349940224;
 A3Nc6_Irw = 998421765;
 QgX_ = -1858347552;
 uMfHM6Z = -1723829633;
 ifigu3 = 1010049136;
 a7dh_0 = -266952393;
 qQ98U1J = -899801381;
 XZiBXO68 = 2138415303;
 L3l9sKo = -1680954167;

QLw62octA4 = (unsigned long)(PulDRKK);
try
{
if (r3iIK == RCb_4Nh4)
{
hGfG24it = (unsigned long)(p2q_KG);
}
else
{
try
{
H2K_12_ = gY1_((unsigned short)1145490131, A3Nc6_Irw);
}
catch (int& errorCode)
{
dw8U7suc--;
}
}
}
catch (int& errorCode)
{
DHWRW2mI = F_HQFry9(-951943820, -1617910784, H2K_12_, IB7Z0n76BL, Wy9cOAtyf);
}
switch (mMGa5Qf)
{
case 'a':
case 'A':
qI4Xz <<= 1001653982;
break;
case 'b':
case 'B':
dJ1E9V = aj6A((unsigned char)2029377970, QW82e1I, Xf3LpiD2);
break;
case 'c':
case 'C':
dJ1E9V /= V871Gk;
break;
case 'd':
case 'D':
O52uZX = (int)abs(fdiH);
break;
case 'e':
case 'E':
Xf3LpiD2 = bcXu | MVf825N;
break;
case 'f':
case 'F':
ifigu3 = gY1_((unsigned short)668449417, (unsigned char)1250303525);
break;
case 'g':
case 'G':
E9gTW24__ = Xf3LpiD2 << P6n28;
break;
default:
SFDI = gY1_((unsigned short)-1543157561, cv07_);
break;
}
ZriOV = (unsigned long)(h5ol7a);

for (int i = 10; i >= 0; i--)
{
mMGa5Qf = (unsigned long)(a2N1);
}
fdiH = (int)abs(O52uZX);
try
{
{
int j = 0;
do
{
std::cout << "fX3iz" << "plP1" << aj6A((unsigned char)-1198800674, (short)-2145919116, Am6e40Mx45) << "ZV6QP78_1A" << std::endl;
j++;
}
while (j < 9);
}
}
catch (int& errorCode)
{
os5nn = gY1_(os5nn, -538132272);
}

if ((vZXzbC2D / F_HQFry9(os5nn, (char)1728370100, (unsigned short)1299985912, G49nHFmsaL, RCb_4Nh4)) <= 1796754287)
{
if ((gY1_((unsigned short)1015474903, (unsigned char)-744242847) / F_HQFry9(os5nn, (char)-37351902, p8nGc34, G49nHFmsaL, RCb_4Nh4)) != 1257720530)
{
ec72z7JE %= 756290609;
}
}

TrK3F6R = aj6A(-1461443946, 808857178, XZiBXO68);

for (int i = 6; i >= 0; --i)
{
try
{
try
{
QLw62octA4 = gY1_((unsigned short)606977606, (unsigned char)1815544693);
}
catch (int& errorCode)
{
GJJ5PEi96 = gY1_(1893754800, r_bn76);
}
}
catch (int& errorCode)
{
s0mggB = I50x_Hx3O & SCTiA2Zl;
}
}

{
int i = 7;
while (i >= 0)
{
i--;
std::cout << gY1_(GJJ5PEi96, (unsigned char)-744648438) << z8q40MbrX << "j8pUjO" << std::endl;
}
}
if (cv07_ != 499988981)
{
vZXzbC2D = aj6A((unsigned char)-657366972, (short)-1238462668, fdiH);
}
else
{
try
{
std::cout << QLw62octA4 << std::endl;
}
catch (int& errorCode)
{
YrKZK = DHWRW2mI >> 31273740;
}
}

ZuHp00x = Zi41E6 << TrK3F6R;
return 0;
}

char	gY1_(unsigned short	gm_t,unsigned char	CJp2)	{
	return	(char)-266799684;
}

long long    F_HQFry9(int    GpJ6Mbijnr,char    ia0FO3q23,unsigned short    j9M6qQIWt,unsigned short int    pTPP4_30_,long long    FL7u9_SK)    {
    return    0;
}

unsigned int    aj6A(unsigned char    RbK8_u,short int    FIHy6a1o,short    vVN0E)    {
    return    378334562;
}

