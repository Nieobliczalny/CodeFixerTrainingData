#include <cstdio>
#include <cstdlib>
#include <iostream>
#include <string>
#include <cstdint>
#include <cmath>


using std::cout;
using std::endl;
using std::abs;
using std::ceil;
using std::floor;

unsigned char	PmIyto(short	RN907F,unsigned short	XEUT4L9OB);

unsigned long long	nG5y8B4(unsigned long	Y0ZE45Ag,unsigned char	Ef6Z2,long	y9DYZr_9_,unsigned int	DBLp37_18,short	O212W);

unsigned short    Xa1E92ctxL(unsigned short int    Hu4gqv_32f,long    dk7L54G,unsigned int    T6iJSLk);

long    b7iKu3_L(unsigned char    AJk7);

int main(int argc, char** argv)
{
	unsigned char	cnpOqc;
	cnpOqc	=	(unsigned char)2026670420;
    unsigned short    QhAi;
    QhAi    =    (unsigned short)-1531760540;
 unsigned long long NgXv_O6;
 NgXv_O6 = 1821987683;
 unsigned long x8_VmPW0;
 x8_VmPW0 = 78134380;
	unsigned long	xflxOIi31g;
	xflxOIi31g	=	-1827418971;
  unsigned long  Pn2S78Y;
  Pn2S78Y  =  2141825714;
    char    yrt_F;
    yrt_F    =    (char)1451840416;
    unsigned long    M8H1;
    M8H1    =    513727504;
  long double  QKFKB4cuPG;
  QKFKB4cuPG  =  1222863289;
 unsigned long long Sd3McsbP1;
 Sd3McsbP1 = 1812497697;
 int phye6avNo;
 phye6avNo = -56462800;
	long	z_0HVT;
	z_0HVT	=	355682207;
  long double  r8lnlra;
  r8lnlra  =  -1695431249;
    unsigned long long    hCU9fT;
    hCU9fT    =    1609545901;
    long    e3KZsh8s5g;
    e3KZsh8s5g    =    1608842516;
    int    rmv8C1O9J;
    rmv8C1O9J    =    1133707539;
    unsigned short    X0CL_8l263;
    X0CL_8l263    =    (unsigned short)-1760138209;
  long  LVFd9LbjwV;
  LVFd9LbjwV  =  1196780506;
    long double    PE10I4h;
    PE10I4h    =    790048950;
	unsigned long long	zxif_z;
	zxif_z	=	706324484;
    unsigned int    tI0peZdd;
    tI0peZdd    =    1018120512;
  short  RwrFQ8;
  RwrFQ8  =  (short)380881286;
	short	J4fed4c;
	J4fed4c	=	(short)-760740942;
	unsigned long	kf4Y4IBeVH;
	kf4Y4IBeVH	=	-1590191201;
  char  S36QGymvl;
  S36QGymvl  =  (char)-955202010;
 long long bVxaogj1;
 bVxaogj1 = -336492246;
  long long  FNhdqNq9T;
  FNhdqNq9T  =  1065444681;
	unsigned long	Hf6iQv;
	Hf6iQv	=	-1403950200;
 long I29vKtXb1U;
 I29vKtXb1U = -1918335405;
	long	o9D5;
	o9D5	=	1458927472;
 short IU8aRm;
 IU8aRm = (short)-625728086;
    float    Q24688T13f;
    Q24688T13f    =    -1314317861;
	char	e9aE;
	e9aE	=	(char)1426923122;
    char    v0fmJ_kUAb;
    v0fmJ_kUAb    =    (char)615402058;
  long long  IpqV8w9;
  IpqV8w9  =  376624892;
    unsigned short int    IY1S_qeTbQ;
    IY1S_qeTbQ    =    (unsigned short)-720321826;
    short    r5E4S5Hm;
    r5E4S5Hm    =    (short)50002381;
	long long	l7zyHDI1r;
	l7zyHDI1r	=	365737922;
  double  nGw1wz1;
  nGw1wz1  =  -1503019350;
  double  rmu8fLq7J;
  rmu8fLq7J  =  1721639692;
  int  aQ2Pm;
  aQ2Pm  =  0;
	unsigned char	lDvQ0O64S7;
	lDvQ0O64S7	=	(unsigned char)392413144;
 float oPv6C3FVCC;
 oPv6C3FVCC = -1209575469;
 long yx3_;
 yx3_ = 887390600;
  long long  n_E9T29;
  n_E9T29  =  0;
 int LagJV73_9W;
 LagJV73_9W = 2087659560;
  long long  tq3Wl;
  tq3Wl  =  -880227609;
    short    eA60_o;
    eA60_o    =    (short)463644623;
 long Ag874MM;
 Ag874MM = 1753498771;
  unsigned short int  BLG2RAw;
  BLG2RAw  =  (unsigned short)-291715637;
  short  c6862;
  c6862  =  0;
    long    vMnw1;
    vMnw1    =    400470107;
	unsigned long long	puD8;
	puD8	=	-2005088715;
 unsigned int wGAnQ6;
 wGAnQ6 = 1574664686;

r5E4S5Hm = b7iKu3_L(r5E4S5Hm);
Q24688T13f = b7iKu3_L((unsigned char)1262961746);

{
int i = 0;
do
{
yx3_ = (int)abs(LVFd9LbjwV);
++i;
}
while (i < 6);
}

{
int i = 0;
do
{
i++;
QhAi = PmIyto((short)2091154445, (unsigned short)2094137813);
}
while (i < 10);
}
try
{
try
{
switch (n_E9T29)
{
case 0:
cnpOqc = nG5y8B4(-214723884, (unsigned char)1555467049, e9aE, IU8aRm, tq3Wl);
break;
case 1:
QKFKB4cuPG = (int)floor(r8lnlra);
break;
case 2:
tq3Wl &= I29vKtXb1U;
break;
case 3:
e3KZsh8s5g = I29vKtXb1U ^ 1496092314;
break;
case 4:
n_E9T29 = Xa1E92ctxL(BLG2RAw, oPv6C3FVCC, n_E9T29);
break;
case 5:
l7zyHDI1r = NgXv_O6 & 1211152048;
break;
default:
l7zyHDI1r = PmIyto((short)-575857801, oPv6C3FVCC);
break;
}
}
catch (int& errorCode)
{
PE10I4h = (int)floor(oPv6C3FVCC);
}
}
catch (int& errorCode)
{
r5E4S5Hm = (int)abs(Ag874MM);
}

cout << nG5y8B4(l7zyHDI1r, (unsigned char)1884012218, -1942273361, I29vKtXb1U, X0CL_8l263) << l7zyHDI1r << aQ2Pm << PmIyto((short)-46034147, (unsigned short)-1527302626) << endl;

IpqV8w9 = b7iKu3_L(IpqV8w9);
e3KZsh8s5g = (long)(v0fmJ_kUAb);

lDvQ0O64S7++;
r8lnlra = b7iKu3_L((unsigned char)1537620135);

IY1S_qeTbQ--;

switch (BLG2RAw)
{
case 'a':
case 'A':
wGAnQ6 &= RwrFQ8;
break;
}
for (int i = 2; i >= 0; i--)
{
for (int j = 4; j >= 0; j--)
{
try
{
LVFd9LbjwV = PmIyto((short)-2037902245, (unsigned short)-876353314);
}
catch (int& errorCode)
{
IU8aRm = (oPv6C3FVCC) ? r5E4S5Hm : PE10I4h;
}
}
}

if ((b7iKu3_L((unsigned char)234260875) / c6862) == -1401159158)
{
{
int i = 1;
while (i >= 0)
{
i--;
try
{
nGw1wz1 = (int)ceil(rmu8fLq7J);
}
catch (int& errorCode)
{
BLG2RAw = PmIyto(1353168993, -671021108);
}
}
}
}
{
int i = 0;
while (i < 1)
{
if (o9D5)
{
try
{
M8H1 = PmIyto(M8H1, (unsigned short)1514240837);
}
catch (int& errorCode)
{
l7zyHDI1r = nG5y8B4(1983238068, M8H1, kf4Y4IBeVH, NgXv_O6, 548055975);
}
}
else
{
try
{
LagJV73_9W = (int)abs(n_E9T29) + (375576350);
}
catch (int& errorCode)
{
r8lnlra = (int)floor(oPv6C3FVCC);
}
}
i++;
}
}
if (rmu8fLq7J)
{
nGw1wz1 = (double)(Hf6iQv);
}
else
{
try
{
c6862 = Xa1E92ctxL((unsigned short)334025687, PE10I4h, 1209234308);
}
catch (int& errorCode)
{
rmu8fLq7J = (int)floor(nGw1wz1);
}
}

for (int i = 0; i < 4; ++i)
{
c6862 = Xa1E92ctxL((unsigned short)1943737142, r8lnlra, v0fmJ_kUAb);
}
return 0;
}

unsigned char	PmIyto(short	RN907F,unsigned short	XEUT4L9OB)
{
	return	(unsigned char)-1664714290;
}

unsigned long long	nG5y8B4(unsigned long	Y0ZE45Ag,unsigned char	Ef6Z2,long	y9DYZr_9_,unsigned int	DBLp37_18,short	O212W)	{
	return	-879972846;
}

unsigned short    Xa1E92ctxL(unsigned short int    Hu4gqv_32f,long    dk7L54G,unsigned int    T6iJSLk)
{
    return    0;
}

long    b7iKu3_L(unsigned char    AJk7)
{
    return    509772782;
}

