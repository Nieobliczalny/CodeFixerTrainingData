#include <cstdlib>
#include <cmath>
#include <iostream>
#include <cstdio>
#include <cstdint>
#include <string>


using namespace std;

long long	Vusf(unsigned int	va0K5E)	{
	return	1718761320;
}

unsigned int    YBHoXD5()    {
    return    0;
}

short int    uk77(unsigned long long    oEyXta9B)
{
    return    -245322356;
}

int main(int argc, char** argv)
{
	char	Y299Ae	=	1998734905,	AY9pHg	=	-1589483504,	ZadxF	=	-280372417;
 short int NO375wb7 = 0, IN5rVPY = 0;

std::cout << "DCF2D" << YBHoXD5() << std::endl;

if (AY9pHg >= -147165676)
{
if ((IN5rVPY) && ((ZadxF) || (ZadxF > 674874433)))
{
IN5rVPY = YBHoXD5();
}
else
{
try
{
ZadxF /= Y299Ae;
}
catch (int& errorCode)
{
ZadxF++;
}
}
}

AY9pHg = (char)(Y299Ae);
if (AY9pHg & NO375wb7)
{
if (IN5rVPY & NO375wb7)
{
NO375wb7 = (int)abs(ZadxF) * 0;
}
else
{
{
int k = 4;
do
{
NO375wb7 = (int)abs(AY9pHg) * 0;
k--;
}
while (k >= 0);
}
}
}

AY9pHg = (char)(IN5rVPY + (2111419801));
for (int i = 3; i >= 0; --i)
{
AY9pHg = (int)abs(IN5rVPY) + (-1385090110);
}

try
{
if (IN5rVPY == NO375wb7)
{
if (Y299Ae)
{
ZadxF = uk77(960144544);
}
else
{
Y299Ae = IN5rVPY << 1514279553;
}
}
}
catch (int& errorCode)
{
Y299Ae = Vusf(28510100);
}
if (Y299Ae == -1288564911)
{
if (AY9pHg & 784865327)
{
AY9pHg ^= 2004081888;
}
else
{
if (Y299Ae)
{
AY9pHg = (Y299Ae >= -389589576) ? uk77(NO375wb7) : -1568027996;
}
else
{
AY9pHg = (Y299Ae < -88587419) ? YBHoXD5() : 752743781;
}
}
}
else
{
try
{
std::cout << NO375wb7 << "d2_zf" << "JMrAVOJ4j2" << std::endl;
}
catch (int& errorCode)
{
AY9pHg = Vusf(IN5rVPY);
}
}
IN5rVPY = (int)abs(ZadxF) * 0;

if ((NO375wb7) && (NO375wb7 <= Y299Ae))
{
try
{
if (ZadxF <= -486036873)
{
AY9pHg /= 2042154536;
}
else
{
ZadxF = Vusf(1663151342);
}
}
catch (int& errorCode)
{
AY9pHg = uk77(103459085);
}
}
else if (ZadxF > -1846471459)
{
if (NO375wb7 != -1742359762)
{
if (Y299Ae == -1988440205)
{
ZadxF = Vusf(IN5rVPY);
}
else if (IN5rVPY >= 212512198)
{
IN5rVPY = (int)abs(Y299Ae) * 0;
}
else if ((NO375wb7 == 1969685310) && ((NO375wb7 == -1836336634) && (IN5rVPY > 865890317)))
{
ZadxF = uk77(ZadxF);
}
else
{
ZadxF = ZadxF << Y299Ae;
}
}
else if (NO375wb7 < Y299Ae)
{
if (Y299Ae < -242017900)
{
ZadxF = (int)abs(Y299Ae);
}
else if (AY9pHg != 395241556)
{
Y299Ae %= AY9pHg;
}
else if (IN5rVPY >= -384704564)
{
NO375wb7 = (int)abs(Y299Ae) * 0;
}
else if (((IN5rVPY & AY9pHg) && (NO375wb7 & 1638328371)) || (NO375wb7 == -708523980))
{
ZadxF = YBHoXD5() / IN5rVPY;
}
else if ((AY9pHg <= IN5rVPY) && ((AY9pHg > IN5rVPY) || (NO375wb7 > -896932542)))
{
ZadxF = AY9pHg >> Y299Ae;
}
else
{
IN5rVPY = (int)abs(ZadxF) * 0;
}
}
else if (ZadxF & IN5rVPY)
{
{
int k = 10;
do
{
k--;
AY9pHg = uk77(AY9pHg);
}
while (k >= 0);
}
}
else if (NO375wb7 > 991015291)
{
if ((Y299Ae & ZadxF) && (ZadxF & 119215027))
{
NO375wb7 = YBHoXD5();
}
}
else if (Y299Ae >= -1312170696)
{
std::cout << YBHoXD5() << Vusf(1845597138) << std::endl;
}
else
{
if (AY9pHg <= 2015375011)
{
NO375wb7 = YBHoXD5();
}
else
{
NO375wb7 = YBHoXD5();
}
}
}
else if ((ZadxF != Y299Ae) && (NO375wb7 > 1140748043))
{
{
int j = 5;
while (j >= 0)
{
ZadxF = uk77(NO375wb7);
--j;
}
}
}
else if (ZadxF & Y299Ae)
{
{
int j = 9;
while (j >= 0)
{
if (((NO375wb7) && (NO375wb7 == -1982233300)) && (IN5rVPY > 1859876784))
{
Y299Ae = ZadxF << AY9pHg;
}
j--;
}
}
}
else if ((Y299Ae > -1922658867) || (NO375wb7 != -1303128188))
{
if (IN5rVPY > 288543230)
{
try
{
AY9pHg = IN5rVPY - NO375wb7;
}
catch (int& errorCode)
{
AY9pHg = Vusf(-120766813);
}
}
else
{
if (IN5rVPY <= -121674576)
{
Y299Ae = uk77(-1038530718) / NO375wb7;
}
}
}
else if (ZadxF != -326953622)
{
{
int j = 0;
do
{
j++;
AY9pHg = uk77(AY9pHg);
}
while (j < 6);
}
}
else
{
Y299Ae = uk77(IN5rVPY);
}
ZadxF = uk77(-1834995710);

{
int i = 0;
while (i < 4)
{
try
{
if (Y299Ae <= IN5rVPY)
{
IN5rVPY = YBHoXD5();
}
}
catch (int& errorCode)
{
AY9pHg = (int)abs(Y299Ae);
}
i++;
}
}

std::cout << Vusf(-1437857766) << AY9pHg << NO375wb7 << "hhT1A" << std::endl;

{
int i = 10;
do
{
if (Y299Ae != 528411050)
{
Y299Ae = (char)(IN5rVPY + (2025777236));
}
else
{
switch (Y299Ae)
{
case 0:
Y299Ae &= ZadxF;
break;
case 1:
IN5rVPY = YBHoXD5();
break;
case 2:
AY9pHg >>= 1440284453;
break;
case 3:
Y299Ae %= ZadxF;
break;
case 4:
NO375wb7 = YBHoXD5();
break;
case 5:
Y299Ae = Vusf(Y299Ae) / ZadxF;
break;
}
}
i--;
}
while (i >= 0);
}

{
int i = 8;
do
{
--i;
Y299Ae -= NO375wb7;
}
while (i >= 0);
}

if (YBHoXD5() == 0)
{
	ZadxF = NO375wb7 / YBHoXD5();
}
for (int i = 0; i < 5; i++)
{
AY9pHg = uk77(-832116771);
}
{
int i = 0;
while (i < 2)
{
i++;
try
{
{
int k = 10;
while (k >= 0)
{
k--;
ZadxF /= 462821025;
}
}
}
catch (int& errorCode)
{
NO375wb7 = YBHoXD5();
}
}
}

{
int i = 1;
while (i >= 0)
{
{
int j = 0;
while (j < 1)
{
{
int k = 0;
do
{
k++;
IN5rVPY = YBHoXD5();
}
while (k < 3);
}
j++;
}
}
i--;
}
}
{
int i = 0;
while (i < 8)
{
i++;
{
int j = 8;
while (j >= 0)
{
j--;
try
{
AY9pHg |= ZadxF;
}
catch (int& errorCode)
{
ZadxF = Vusf(IN5rVPY);
}
}
}
}
}

ZadxF = (char)(IN5rVPY + (2002975670));

for (int i = 5; i >= 0; i--)
{
NO375wb7 = (short int)(IN5rVPY);
}
std::cout << uk77(-418785087) << std::endl;

{
int i = 4;
while (i >= 0)
{
AY9pHg -= ZadxF;
--i;
}
}

try
{
if (ZadxF == -1060055953)
{
{
int k = 0;
do
{
ZadxF = Vusf(361290509);
++k;
}
while (k < 9);
}
}
}
catch (int& errorCode)
{
AY9pHg = (int)abs(ZadxF);
}

if (((ZadxF <= IN5rVPY) || (NO375wb7 == 735437922)) && ((Y299Ae > 277554913) || (AY9pHg > ZadxF)))
{
try
{
{
int k = 7;
do
{
AY9pHg = (int)abs(Y299Ae);
k--;
}
while (k >= 0);
}
}
catch (int& errorCode)
{
Y299Ae = ZadxF / 1163266327;
}
}
else
{
Y299Ae--;
}
{
int i = 4;
do
{
AY9pHg = Vusf(-1772752391);
i--;
}
while (i >= 0);
}
{
int i = 0;
while (i < 9)
{
i++;
try
{
{
int k = 3;
do
{
Y299Ae |= ZadxF;
k--;
}
while (k >= 0);
}
}
catch (int& errorCode)
{
ZadxF = Vusf(ZadxF);
}
}
}

try
{
IN5rVPY = YBHoXD5();
}
catch (int& errorCode)
{
Y299Ae %= 1286647464;
}

switch (ZadxF)
{
case 'a':
case 'A':
AY9pHg = IN5rVPY % 869131823;
break;
case 'b':
case 'B':
IN5rVPY = (int)abs(AY9pHg) * 0;
break;
case 'c':
case 'C':
Y299Ae <<= AY9pHg;
break;
case 'd':
case 'D':
ZadxF = uk77(1026711252);
break;
case 'e':
case 'E':
Y299Ae--;
break;
case 'f':
case 'F':
Y299Ae = uk77(-388067895);
break;
case 'g':
case 'G':
Y299Ae = Y299Ae - AY9pHg;
break;
}
switch (Y299Ae)
{
case 'a':
case 'A':
ZadxF = IN5rVPY - 14105172;
break;
case 'b':
case 'B':
NO375wb7 = YBHoXD5();
break;
case 'c':
case 'C':
NO375wb7 = (int)abs(IN5rVPY);
break;
case 'd':
case 'D':
Y299Ae--;
break;
case 'e':
case 'E':
ZadxF--;
break;
case 'f':
case 'F':
AY9pHg = Vusf(AY9pHg);
break;
case 'g':
case 'G':
IN5rVPY = YBHoXD5();
break;
}
ZadxF = (char)(NO375wb7 + (290415462));

return 0;
}

