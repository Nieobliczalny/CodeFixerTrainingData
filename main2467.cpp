#include <cstdio>
#include <cstdint>
#include <string>
#include <iostream>
#include <cstdlib>
#include <cmath>


using namespace std;

char    erRgz4z8(unsigned long    e30m,unsigned short    CdOz1mD,long    iC7k1yS4Nn,unsigned long long    k21MuK3);

int  v60k9tYmYo(long  f4kwvu,unsigned int  z2g4Uc4N1,long long  sRa39IGf,int  M0dD3L6PH);

unsigned short int  qHK1(unsigned long  ogH6TvGr93,unsigned short  H_Uj0h4Yj,unsigned short int  iGF_K0d,unsigned int  tAJu,char  tZ_E6BFHYX);

int main(int argc, char** argv) {
    short    d9lY,    aU97,    aXba2wD2Vm,    bx2S9303db;
    d9lY    =    0;
    aU97    =    0;
    aXba2wD2Vm    =    0;
    bx2S9303db    =    0;
  unsigned short  Bd2_9pF7,  HynqoAIA9,  Uk4Yev,  CXX3ZJQ884,  k4gD0,  CK7agKYKL1,  qpaS_Z,  PPH16S,  c_ESX0,  GG0Ko3Z5,  T54fJ,  sz_c0,  r2Q69I,  fYs_Whe,  azi2zo8ikX,  Y3qbPcJiP2,  kTH3rHT8,  ucxud,  Ur3C45cw,  Bxe4p,  kWkq,  cFSmLai,  u_fvR,  Mndyn4HK18,  jhXgH63S57,  KnX7q8Z383,  CIf_HGP,  xMqPkYDmEZ;
  Bd2_9pF7  =  (unsigned short)1220767145;
  HynqoAIA9  =  (unsigned short)1596053174;
  Uk4Yev  =  (unsigned short)-1542244803;
  CXX3ZJQ884  =  (unsigned short)-1436342765;
  k4gD0  =  (unsigned short)-657314505;
  CK7agKYKL1  =  (unsigned short)-1368603041;
  qpaS_Z  =  -75943596;
  PPH16S  =  1857938510;
  c_ESX0  =  -1899619562;
  GG0Ko3Z5  =  1766398776;
  T54fJ  =  (unsigned short)-1089554240;
  sz_c0  =  (unsigned short)-1949441571;
  r2Q69I  =  (unsigned short)-1644182081;
  fYs_Whe  =  (unsigned short)801331997;
  azi2zo8ikX  =  (unsigned short)791713298;
  Y3qbPcJiP2  =  (unsigned short)-1866107126;
  kTH3rHT8  =  1031231868;
  ucxud  =  828612681;
  Ur3C45cw  =  -1318492122;
  Bxe4p  =  -1233141186;
  kWkq  =  (unsigned short)1537938073;
  cFSmLai  =  (unsigned short)-1846591667;
  u_fvR  =  (unsigned short)1388333982;
  Mndyn4HK18  =  (unsigned short)1445027827;
  jhXgH63S57  =  (unsigned short)185086712;
  KnX7q8Z383  =  (unsigned short)357739182;
  CIf_HGP  =  -2000949365;
  xMqPkYDmEZ  =  -466931667;

sz_c0 = qpaS_Z / qHK1(-952813388, -190718009, -372893562, 704779289, k4gD0);
std::cout << qHK1(6251973, HynqoAIA9, (unsigned short)-1316878129, CXX3ZJQ884, k4gD0) << erRgz4z8(-815549232, HynqoAIA9, Uk4Yev, CXX3ZJQ884) << "id6v0676U" << qpaS_Z << "sIH1YU" << std::endl;

c_ESX0 = v60k9tYmYo(c_ESX0, HynqoAIA9, cFSmLai, -943599758);

for (int i = 4; i >= 0; i--)
{
std::cout << T54fJ << std::endl;
}
switch (aU97)
{
case 'a':
case 'A':
cFSmLai = v60k9tYmYo(-417315199, -1898082676, 1595694047, -1594885499);
break;
case 'b':
case 'B':
GG0Ko3Z5 = KnX7q8Z383 | 1734155068;
break;
case 'c':
case 'C':
qpaS_Z = T54fJ << 953625492;
break;
case 'd':
case 'D':
d9lY = qHK1(HynqoAIA9, (unsigned short)797229054, (unsigned short)-1022400987, 1909112741, (char)321139651);
break;
}

for (int i = 6; i >= 0; --i)
{
fYs_Whe = (unsigned short)(Uk4Yev);
}

kTH3rHT8 = (unsigned short)(xMqPkYDmEZ);
return 0;
}

char    erRgz4z8(unsigned long    e30m,unsigned short    CdOz1mD,long    iC7k1yS4Nn,unsigned long long    k21MuK3)
{
    return    (char)-67356662;
}

int  v60k9tYmYo(long  f4kwvu,unsigned int  z2g4Uc4N1,long long  sRa39IGf,int  M0dD3L6PH)
{
  return  892888882;
}

unsigned short int  qHK1(unsigned long  ogH6TvGr93,unsigned short  H_Uj0h4Yj,unsigned short int  iGF_K0d,unsigned int  tAJu,char  tZ_E6BFHYX)  {
  return  0;
}

