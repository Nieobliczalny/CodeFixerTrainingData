#include <cmath>
#include <cstdlib>
#include <cstdint>
#include <cstdio>
#include <string>
#include <iostream>

#define    Bj_g7Id60q    (739556897)
#define	zXN_	(0)

using namespace std;

long long  zWJfcuU4u();

char	w20Q(unsigned int	s0z236pXMp,int	L4eIA,unsigned char	AdnF9NRk_3,unsigned short	WB1Ui,long long	kk4hN56PdR);

unsigned short int    KGLy4(short    T9vp9jEGy1,unsigned long long    Z7_z9,short int    IJJukG1I7M);

short int    x0lbGvZGjx(unsigned long    Ufi0,unsigned short    i8QSL,short    x67aK6NIRw,unsigned short int    J_ERKyf,long    I32Sw3I);

int main(int argc, char** argv) {
  float  qXrNvuL1Wo;
  qXrNvuL1Wo  =  -1123978634;
  unsigned short  Ua170b;
  Ua170b  =  (unsigned short)-1750314678;
 unsigned short l6xa60r;
 l6xa60r = (unsigned short)548493347;
    float    Px4Jp9wF_;
    Px4Jp9wF_    =    -1070329979;
 double fzbYkC;
 fzbYkC = -1006368586;
	unsigned long	xyf4PArvn;
	xyf4PArvn	=	-2019387760;
 unsigned short CHa0tWVJ;
 CHa0tWVJ = (unsigned short)-1372574402;
  unsigned long long  jAKv0syn;
  jAKv0syn  =  -937191503;
    char    KapC;
    KapC    =    (char)1943228938;
  int  Y1SY446;
  Y1SY446  =  194147750;
  float  yOZRC;
  yOZRC  =  394648451;
    unsigned short int    zZa6;
    zZa6    =    (unsigned short)-148051407;
	unsigned long long	zEM2kB;
	zEM2kB	=	-1380248867;
    short    ZVolz;
    ZVolz    =    (short)-1856783149;
    short    rj7D93n;
    rj7D93n    =    (short)-746802193;
	short int	QDNUQaa4;
	QDNUQaa4	=	(short)-394268122;
	float	xdnJ0HVIN;
	xdnJ0HVIN	=	-1433174581;
 char IX2kIgJ;
 IX2kIgJ = (char)-1711418834;
	short	w96vRtKZvg;
	w96vRtKZvg	=	(short)-1040847296;
	int	G9_ehRf;
	G9_ehRf	=	-769993947;
    unsigned short    J60moPIuPZ;
    J60moPIuPZ    =    (unsigned short)-354919413;
	char	RyPqU3t8X;
	RyPqU3t8X	=	(char)2014289771;
    float    x64d_9S;
    x64d_9S    =    942062903;
	int	V8C8_DPV3U;
	V8C8_DPV3U	=	-69168356;
 unsigned long long lRBx;
 lRBx = -1863118028;
	unsigned long	JyjnQgwL;
	JyjnQgwL	=	-43014795;
 unsigned short f31XW;
 f31XW = (unsigned short)-879719284;
    short    kNk679_bme;
    kNk679_bme    =    (short)-90704423;
    unsigned short int    efdK;
    efdK    =    (unsigned short)-881881063;
	unsigned long long	G7HJKccE;
	G7HJKccE	=	850993383;
	short	Gn9JY;
	Gn9JY	=	0;
	long long	IjcjVI9;
	IjcjVI9	=	437795881;
	char	fbWj8v49K;
	fbWj8v49K	=	(char)-581035919;
  long double  B5fz;
  B5fz  =  1441854414;
  unsigned short  gAzvQ41;
  gAzvQ41  =  (unsigned short)260923628;
  unsigned int  NN_239WQ7;
  NN_239WQ7  =  438688135;
    unsigned char    g0Tyvm;
    g0Tyvm    =    (unsigned char)238143477;
  unsigned short  Au22;
  Au22  =  (unsigned short)-975144187;
 long long sLBWw_7M;
 sLBWw_7M = 1142225780;
    unsigned long long    E9r02Nnhq7;
    E9r02Nnhq7    =    -1788117434;
    int    dd4wl;
    dd4wl    =    -528887870;
    unsigned short int    bZBQY4a17s;
    bZBQY4a17s    =    (unsigned short)706641717;
    unsigned long long    T1d63l;
    T1d63l    =    -636077149;
 short vIfbo;
 vIfbo = (short)-16715683;
  unsigned short int  dw496012;
  dw496012  =  0;
 short ZSiJ;
 ZSiJ = (short)1721580618;
  unsigned int  x_m15;
  x_m15  =  -461653852;
 double EMu216;
 EMu216 = 211639880;
	unsigned char	ZI__;
	ZI__	=	(unsigned char)734459486;
	unsigned long long	H0Ad86lA0C;
	H0Ad86lA0C	=	0;
 unsigned short int rx1_bBw;
 rx1_bBw = (unsigned short)1939241607;
 unsigned short int SMUIu1hbL;
 SMUIu1hbL = (unsigned short)-1841180626;
	int	v7oa;
	v7oa	=	1705611312;
 unsigned long long mKN7v8;
 mKN7v8 = -1080555546;
 float SgXCc;
 SgXCc = -1023380401;
    short int    D9K4x;
    D9K4x    =    (short)1043929485;
  unsigned int  nuau8a0XA;
  nuau8a0XA  =  -2082515291;
  float  vJK_SSO7;
  vJK_SSO7  =  -1019752094;
  long double  s5o_iQ4A4;
  s5o_iQ4A4  =  -418282542;
    char    C7I_z6pA8;
    C7I_z6pA8    =    (char)-1194210670;
    unsigned long long    mqinPv7Vr;
    mqinPv7Vr    =    -564331545;
 unsigned short int wv1fH9O;
 wv1fH9O = (unsigned short)1290774636;
  char  sLzHKR;
  sLzHKR  =  (char)-1770374365;
 unsigned short Au2_qa;
 Au2_qa = (unsigned short)-2072526188;
 unsigned short QGoNa;
 QGoNa = (unsigned short)787352952;
 float Bq6Ozh;
 Bq6Ozh = -1151463152;
 long NeY7A;
 NeY7A = 511901351;
 long long en5n5X_;
 en5n5X_ = 1328181151;
  double  IJ7HALcD;
  IJ7HALcD  =  1357622825;
 unsigned char k5Uhbt9;
 k5Uhbt9 = (unsigned char)-1145831696;
    unsigned long    E26BYm85Y;
    E26BYm85Y    =    -1767837078;
 unsigned long long VeP9j0iq;
 VeP9j0iq = -1587223588;
  unsigned short int  uDQ7;
  uDQ7  =  (unsigned short)753265612;
 double x6gQ;
 x6gQ = -295446848;

if (gAzvQ41 >= IjcjVI9)
{
JyjnQgwL = (unsigned long)(Au22);
}

if (IjcjVI9 & 184624558)
{
qXrNvuL1Wo = (int)ceil(s5o_iQ4A4);
}
else if ((xdnJ0HVIN >= JyjnQgwL) && ((V8C8_DPV3U < -946546777) && (Px4Jp9wF_ < Gn9JY)))
{
{
int j = 1;
while (j >= 0)
{
V8C8_DPV3U = (int)(Bj_g7Id60q);
j--;
}
}
}
else if (ZSiJ)
{
switch (JyjnQgwL)
{
case 0:
Px4Jp9wF_ = (int)ceil(yOZRC);
break;
case 1:
s5o_iQ4A4 = (int)floor(x6gQ);
break;
case 2:
xdnJ0HVIN = (int)ceil(x6gQ);
break;
case 3:
SgXCc = (int)floor(IJ7HALcD);
break;
case 4:
EMu216 = KGLy4(EMu216, -21842098, (short)-1437420340);
break;
case 5:
QGoNa >>= 925610244;
break;
}
}
else
{
G7HJKccE %= nuau8a0XA;
}

if (sLBWw_7M == Px4Jp9wF_)
{
{
int j = 0;
do
{
j++;
{
int k = 0;
do
{
k++;
V8C8_DPV3U = Ua170b / nuau8a0XA;
}
while (k < 1);
}
}
while (j < 5);
}
}
else if (vJK_SSO7 > CHa0tWVJ)
{
EMu216 = (int)floor(xdnJ0HVIN);
}
else
{
IjcjVI9 %= lRBx;
}

for (int i = 2; i >= 0; --i)
{
uDQ7 = (unsigned short int)(nuau8a0XA);
}

switch (V8C8_DPV3U)
{
case 'a':
case 'A':
rj7D93n = x0lbGvZGjx(-1812800381, f31XW, (short)-93831987, (unsigned short)1143556827, -396059339);
break;
default:
l6xa60r = ZSiJ >> kNk679_bme;
break;
}
SMUIu1hbL = (unsigned short int)(D9K4x);

k5Uhbt9 = fbWj8v49K / zWJfcuU4u();

if (v7oa & 669157059)
{
D9K4x = (zZa6 > C7I_z6pA8) ? wv1fH9O : (short)597683002;
}
else
{
vIfbo = KGLy4((short)-2018646389, V8C8_DPV3U, x64d_9S);
}
switch (en5n5X_)
{
case 'a':
H0Ad86lA0C = zWJfcuU4u();
break;
case 'b':
QDNUQaa4 = (int)abs(Y1SY446);
break;
case 'c':
SgXCc = x0lbGvZGjx(SgXCc, IJ7HALcD, (short)129725855, gAzvQ41, en5n5X_);
break;
}
for (int i = 0; i < 6; i++)
{
{
int j = 0;
do
{
zZa6 = (unsigned short int)(VeP9j0iq);
j++;
}
while (j < 2);
}
}

cout << lRBx << "xTq9I43V" << endl;

SMUIu1hbL = (unsigned short int)(Px4Jp9wF_);
{
int i = 4;
do
{
{
int j = 2;
do
{
try
{
x6gQ = (int)ceil(Bq6Ozh);
}
catch (int& errorCode)
{
xdnJ0HVIN = (int)ceil(fzbYkC);
}
--j;
}
while (j >= 0);
}
--i;
}
while (i >= 0);
}

cout << NeY7A << endl;

B5fz = (int)ceil(vJK_SSO7);

return 0;
}

long long  zWJfcuU4u()  {
  return  0;
}

char	w20Q(unsigned int	s0z236pXMp,int	L4eIA,unsigned char	AdnF9NRk_3,unsigned short	WB1Ui,long long	kk4hN56PdR)
{
	return	(char)713607601;
}

unsigned short int    KGLy4(short    T9vp9jEGy1,unsigned long long    Z7_z9,short int    IJJukG1I7M)
{
    return    (unsigned short)-709183804;
}

short int    x0lbGvZGjx(unsigned long    Ufi0,unsigned short    i8QSL,short    x67aK6NIRw,unsigned short int    J_ERKyf,long    I32Sw3I)    {
    return    (short)-2126694999;
}

