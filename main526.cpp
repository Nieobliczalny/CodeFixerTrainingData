#include <cstdint>
#include <iostream>
#include <cmath>
#include <cstdio>
#include <string>
#include <cstdlib>


using namespace std;

unsigned short uk8671s8(int S8kFB56MC)
{
 return (unsigned short)1473532404;
}

char    V96y(unsigned short int    RJcumZG,unsigned char    B_04C_47,unsigned char    Sse7)    {
    return    0;
}

int main() {
  unsigned short int  jrdaScm4_,  o2tCUM,  ii84GEq,  pF843f0,  r_q4,  ziLy5q,  vHQKOGz,  ix2V9m,  S9AA3O0G,  I_SeM,  WR0hCMjYI_,  nIpf,  D79_Y,  cU98C1,  wp0r6ty,  N3vbl_1u2_,  Oht7Eg5WV,  tgg7Tm,  hj2FmA_0,  F4xF,  XRhO,  Kp_Vb,  FI_cp4,  F34Zq,  J2bkvzUd72,  Y0_3n33,  qbl_1u,  K9F06ThB6Z,  Z0fF01ypi1,  ktKxn,  h4n5z3,  O1vK5u,  an1DV_,  Ga7fx9vx,  HF0tU,  B1i4M,  Gz__fEGxG,  IUeItX,  K8Wi9sD_,  D6Zi5Lk7,  TWE1D2,  CfHU6z,  l523YyO,  qJPKm14R,  bH6D_w_,  Gj_GCj,  nfoY0zm6FL,  eT3O9T4c3,  Y5UggBX97,  fiwXpv11,  f_Y_J8Q,  R4IePG_87,  T2n6575,  WVkzn,  G6Hjj1iz0d,  bV55T71,  poqRT,  H9nEyLL0,  k845Q,  POT2IVTd6,  VS3z;
    jrdaScm4_  =  o2tCUM  =  ii84GEq  =  pF843f0  =  r_q4  =  ziLy5q  =  vHQKOGz  =  ix2V9m  =  S9AA3O0G  =  I_SeM  =  WR0hCMjYI_  =  nIpf  =  D79_Y  =  cU98C1  =  wp0r6ty  =  N3vbl_1u2_  =  Oht7Eg5WV  =  tgg7Tm  =  hj2FmA_0  =  F4xF  =  XRhO  =  Kp_Vb  =  FI_cp4  =  F34Zq  =  J2bkvzUd72  =  Y0_3n33  =  qbl_1u  =  K9F06ThB6Z  =  Z0fF01ypi1  =  ktKxn  =  h4n5z3  =  O1vK5u  =  an1DV_  =  Ga7fx9vx  =  HF0tU  =  B1i4M  =  Gz__fEGxG  =  IUeItX  =  K8Wi9sD_  =  D6Zi5Lk7  =  TWE1D2  =  CfHU6z  =  l523YyO  =  qJPKm14R  =  bH6D_w_  =  Gj_GCj  =  nfoY0zm6FL  =  eT3O9T4c3  =  Y5UggBX97  =  fiwXpv11  =  f_Y_J8Q  =  R4IePG_87  =  T2n6575  =  WVkzn  =  G6Hjj1iz0d  =  bV55T71  =  poqRT  =  H9nEyLL0  =  k845Q  =  POT2IVTd6  =  VS3z  =  (unsigned short)3455036;
	long long	a0wou6dbu0,	hePZYti,	d0f06,	U0N4i_A7WM,	oYl2h64;
		a0wou6dbu0	=	hePZYti	=	d0f06	=	U0N4i_A7WM	=	oYl2h64	=	0;

{
int i = 0;
do
{
std::cout << V96y(jrdaScm4_, (unsigned char)329603481, ii84GEq) << "De9IR5YKX1" << std::endl;
i++;
}
while (i < 7);
}

if (hj2FmA_0 != IUeItX)
{
if (((K8Wi9sD_ == CfHU6z) || (F34Zq)) || (eT3O9T4c3 >= N3vbl_1u2_))
{
VS3z = CfHU6z % T2n6575;
}
else if ((an1DV_ < Z0fF01ypi1) && (nfoY0zm6FL & 2034245273))
{
{
int k = 0;
do
{
k++;
wp0r6ty = uk8671s8(445027456) - o2tCUM;
}
while (k < 6);
}
}
else
{
switch (cU98C1)
{
case 0:
qbl_1u = uk8671s8(qbl_1u);
break;
case 1:
d0f06 = V96y((unsigned short)-872842853, (unsigned char)112611865, wp0r6ty);
break;
case 2:
hj2FmA_0 = (F4xF > 1874746507) ? uk8671s8(-836765070) : (unsigned short)-1985475223;
break;
case 3:
hePZYti = V96y(h4n5z3, (unsigned char)2062666828, hePZYti);
break;
case 4:
U0N4i_A7WM = V96y((unsigned short)981747130, (unsigned char)-21303888, (unsigned char)2055766452);
break;
case 5:
hePZYti = V96y((unsigned short)-259774699, bV55T71, (unsigned char)859855620);
break;
case 6:
D79_Y = nIpf % poqRT;
break;
case 7:
hj2FmA_0 = uk8671s8(2044687479);
break;
}
}
}
else
{
HF0tU = (unsigned short int)(Z0fF01ypi1);
}

switch (fiwXpv11)
{
case 0:
hePZYti = V96y((unsigned short)-537006051, jrdaScm4_, F4xF);
break;
case 1:
K9F06ThB6Z = pF843f0 - POT2IVTd6;
break;
case 2:
oYl2h64 = V96y(qJPKm14R, F4xF, ktKxn);
break;
case 3:
S9AA3O0G >>= J2bkvzUd72;
break;
case 4:
Y5UggBX97 *= d0f06;
break;
case 5:
d0f06 = (int)abs(oYl2h64);
break;
case 6:
h4n5z3 ^= I_SeM;
break;
case 7:
B1i4M = l523YyO - F4xF;
break;
}
{
int i = 9;
do
{
i--;
for (int j = 10; j >= 0; j--)
{
FI_cp4 = an1DV_ / F34Zq;
}
}
while (i >= 0);
}

switch (Z0fF01ypi1)
{
case 'a':
case 'A':
D79_Y = V96y((unsigned short)261495015, (unsigned char)1668624378, ktKxn) - 1825667934;
break;
case 'b':
case 'B':
U0N4i_A7WM = V96y((unsigned short)222159835, (unsigned char)367287915, (unsigned char)1005474140);
break;
case 'c':
case 'C':
oYl2h64 = V96y(pF843f0, (unsigned char)-760654428, (unsigned char)1308996664);
break;
case 'd':
case 'D':
hePZYti = V96y((unsigned short)-545673677, (unsigned char)1054325371, -1639128331);
break;
case 'e':
case 'E':
POT2IVTd6 %= T2n6575;
break;
default:
qbl_1u = uk8671s8(qbl_1u);
break;
}
std::cout << 1750259174 / oYl2h64 << std::endl;

for (int i = 0; i < 7; i++)
{
hePZYti = (int)abs(a0wou6dbu0);
}

I_SeM = uk8671s8(I_SeM);

for (int i = 5; i >= 0; i--)
{
jrdaScm4_ = uk8671s8(jrdaScm4_);
}
try
{
{
int j = 9;
do
{
j--;
{
int k = 5;
while (k >= 0)
{
ix2V9m >>= poqRT;
--k;
}
}
}
while (j >= 0);
}
}
catch (int& errorCode)
{
ii84GEq = (N3vbl_1u2_ & 397496843) ? uk8671s8(142666968) : Z0fF01ypi1;
}
oYl2h64 = V96y(an1DV_, (unsigned char)2141004786, (unsigned char)-351012886);
if ((Y0_3n33 >= -1647886892) && ((ziLy5q) || (jrdaScm4_ <= poqRT)))
{
F4xF ^= Z0fF01ypi1;
}
else
{
std::cout << uk8671s8(-2103873149) << I_SeM << "zj_gj9_Jjf" << "sMEqO74_Du" << uk8671s8(-894484104) << std::endl;
}
IUeItX = uk8671s8(IUeItX);
{
int i = 0;
while (i < 3)
{
{
int j = 0;
while (j < 8)
{
j++;
D79_Y = (unsigned short int)(oYl2h64 + (-212742697));
}
}
i++;
}
}

o2tCUM = uk8671s8(-1115997650);
if (cU98C1 >= J2bkvzUd72)
{
std::cout << uk8671s8(-484558594) << uk8671s8(d0f06) << Y0_3n33 << uk8671s8(d0f06) << std::endl;
}
{
int i = 9;
do
{
i--;
a0wou6dbu0 = V96y((unsigned short)-125058853, F4xF, (unsigned char)2077913754);
}
while (i >= 0);
}

{
int i = 0;
while (i < 4)
{
++i;
switch (XRhO)
{
case 0:
U0N4i_A7WM = V96y((unsigned short)-423653435, O1vK5u, vHQKOGz);
break;
case 1:
d0f06 = V96y(CfHU6z, (unsigned char)-352724740, S9AA3O0G);
break;
case 2:
Z0fF01ypi1 = an1DV_ % 304701865;
break;
case 3:
hj2FmA_0 = uk8671s8(1769800211);
break;
}
}
}

if (hePZYti > -1828736413)
{
if (Gj_GCj < 184624807)
{
if (D6Zi5Lk7 > TWE1D2)
{
cU98C1 = B1i4M >> an1DV_;
}
}
}
else
{
if ((hj2FmA_0 <= -1516257644) || ((h4n5z3 < 1488113199) || (vHQKOGz >= k845Q)))
{
{
int k = 7;
do
{
an1DV_ = G6Hjj1iz0d >> K8Wi9sD_;
k--;
}
while (k >= 0);
}
}
else
{
{
int k = 3;
do
{
eT3O9T4c3 = uk8671s8(932263587);
--k;
}
while (k >= 0);
}
}
}

return 0;
}

