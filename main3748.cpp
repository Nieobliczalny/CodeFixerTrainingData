#include <string>
#include <cstdio>
#include <iostream>
#include <cstdint>
#include <cmath>
#include <cstdlib>


using namespace std;

unsigned char	PZV4u4T3X(unsigned int	l3jEx_1m,short	fvJs,unsigned int	SwNBJc7s,unsigned long	K_iK);

char w99h27H(char lcyu2hx,long long Fbq47gZ);

long	xgIU(long long	yTF9jCr_9D,unsigned long	K6xj,long	SmF7g6);

short int S4xNIJte0(int aPMQ9Tq,unsigned int i93Zg_nt,unsigned long MCN5NA,short r6b_krI0C);

char    qqSO6OTKjL(unsigned short int    a0Hd,int    g4K42_x,short    Jep_D36);

int main()
{
  char  oj443;
  oj443  =  (char)167164966;
	unsigned short int	fkU1biro;
	fkU1biro	=	(unsigned short)2023202893;
  unsigned short int  G___16j5;
  G___16j5  =  (unsigned short)-1711804197;
    unsigned char    RxM06I09;
    RxM06I09    =    -745634867;
 double l1g8yow;
 l1g8yow = 1207947479;
 short int j0_rcGo7;
 j0_rcGo7 = (short)2042190234;
  unsigned char  VTl9u;
  VTl9u  =  0;
  long  jtY2_PLa_;
  jtY2_PLa_  =  0;
	char	OCbjYfy98y;
	OCbjYfy98y	=	(char)-921061384;
 unsigned long long Pzy6;
 Pzy6 = -2114530077;
 float oG20kE;
 oG20kE = 119909715;
	unsigned short	Q9P1V;
	Q9P1V	=	(unsigned short)-843888844;
  unsigned short  sNec0S3__A;
  sNec0S3__A  =  (unsigned short)-127840687;
    double    EbnCLb5Ado;
    EbnCLb5Ado    =    1316905083;
	unsigned int	Hl55D5a;
	Hl55D5a	=	393844752;
  long long  RvWfuNtvy;
  RvWfuNtvy  =  813035031;
  unsigned short int  a62qk;
  a62qk  =  (unsigned short)1303539752;
 int i0o9G;
 i0o9G = 1914255187;
 unsigned short OGdGIci5YR;
 OGdGIci5YR = (unsigned short)1983741985;
  float  oTyfCU3l;
  oTyfCU3l  =  -341805283;
	long long	Qg3nH4tN;
	Qg3nH4tN	=	0;
    float    v5kQ;
    v5kQ    =    -1822226543;
 long double jh03WT9Y_;
 jh03WT9Y_ = -1884221740;
  int  H392R4h0;
  H392R4h0  =  -2003341193;
  int  x8BY3;
  x8BY3  =  1086008655;
    unsigned int    Nt6M4j_o9K;
    Nt6M4j_o9K    =    -2106803144;
  int  d58ju;
  d58ju  =  -559526780;
	unsigned int	w0Dab6;
	w0Dab6	=	-2099595491;
 long long sTUQ7_Sl8I;
 sTUQ7_Sl8I = -1020620989;
    unsigned long long    Q89RSS75;
    Q89RSS75    =    -1373579801;
    unsigned long    DpGG6J4DK;
    DpGG6J4DK    =    684965165;
 short VhdI;
 VhdI = (short)651500972;
    long long    qGJh;
    qGJh    =    -1317547152;
    int    l9RAJ0HxTl;
    l9RAJ0HxTl    =    -1152305799;
 unsigned short int h57hzpb_;
 h57hzpb_ = (unsigned short)1122788877;
    unsigned short int    BM2r45M;
    BM2r45M    =    (unsigned short)-1925346733;
 int pU7YF;
 pU7YF = 433521617;

switch (h57hzpb_)
{
case 'a':
case 'A':
G___16j5 = PZV4u4T3X(G___16j5, (short)-2039644763, 1456597857, oG20kE);
break;
case 'b':
case 'B':
DpGG6J4DK = w99h27H(DpGG6J4DK, Q9P1V);
break;
case 'c':
case 'C':
w0Dab6 = VTl9u % 118265089;
break;
case 'd':
case 'D':
v5kQ = (int)floor(l1g8yow);
break;
}

for (int i = 0; i < 10; i++)
{
if (RxM06I09)
{
if (Q89RSS75 < 237795222)
{
l1g8yow = S4xNIJte0(1134950420, Q89RSS75, 1931745838, VhdI);
}
else
{
jh03WT9Y_ = (int)floor(EbnCLb5Ado);
}
}
}
cout << a62qk << "J1dH49" << endl;
{
int i = 9;
while (i >= 0)
{
for (int j = 4; j >= 0; j--)
{
{
int k = 3;
while (k >= 0)
{
--k;
i0o9G = S4xNIJte0(i0o9G, 1798775074, j0_rcGo7, x8BY3);
}
}
}
i--;
}
}
{
int i = 0;
while (i < 2)
{
i++;
switch (j0_rcGo7)
{
case 'a':
v5kQ = (int)ceil(l1g8yow);
break;
case 'b':
Hl55D5a = S4xNIJte0(-1201420723, VTl9u, -1322333860, G___16j5);
break;
case 'c':
H392R4h0 = pU7YF + VTl9u;
break;
case 'd':
OGdGIci5YR--;
break;
case 'e':
jh03WT9Y_ = (int)floor(oG20kE);
break;
case 'f':
RxM06I09 = S4xNIJte0(RxM06I09, a62qk, w0Dab6, fkU1biro);
break;
case 'g':
G___16j5 = xgIU(1108748132, DpGG6J4DK, -547828303);
break;
case 'h':
jtY2_PLa_ = (int)abs(Qg3nH4tN);
break;
case 'i':
oTyfCU3l = PZV4u4T3X(oTyfCU3l, (short)-1803402186, BM2r45M, -2029845013);
break;
default:
l1g8yow = (int)ceil(oTyfCU3l);
break;
}
}
}
for (int i = 0; i < 4; i++)
{
cout << "RbJxSa_6lI" << i0o9G << w99h27H(oTyfCU3l, 1091899857) << oG20kE << endl;
}

qGJh <<= H392R4h0;

if (jtY2_PLa_ == 0)
{
	VhdI = a62qk / jtY2_PLa_;
}

try
{
Qg3nH4tN = (long long)(jh03WT9Y_ * Qg3nH4tN);
}
catch (int& errorCode)
{
VTl9u = qqSO6OTKjL(v5kQ, RvWfuNtvy, 1492598608);
}

{
int i = 3;
while (i >= 0)
{
i--;
Pzy6 = (unsigned long long)(pU7YF);
}
}
i0o9G = (int)abs(sTUQ7_Sl8I);

switch (sNec0S3__A)
{
case 0:
BM2r45M -= 1104992178;
break;
case 1:
Q9P1V = Qg3nH4tN + DpGG6J4DK;
break;
case 2:
Nt6M4j_o9K >>= i0o9G;
break;
case 3:
oG20kE = (int)floor(l1g8yow);
break;
case 4:
BM2r45M = S4xNIJte0(667218876, G___16j5, w0Dab6, (short)1449392594);
break;
case 5:
j0_rcGo7 = (int)abs(oj443);
break;
case 6:
OGdGIci5YR = BM2r45M / H392R4h0;
break;
default:
h57hzpb_ = xgIU(13450297, 2129666732, 845847900);
break;
}

return 0;
}

unsigned char	PZV4u4T3X(unsigned int	l3jEx_1m,short	fvJs,unsigned int	SwNBJc7s,unsigned long	K_iK)	{
	return	(unsigned char)-1082971861;
}

char w99h27H(char lcyu2hx,long long Fbq47gZ) {
 return (char)-485381821;
}

long	xgIU(long long	yTF9jCr_9D,unsigned long	K6xj,long	SmF7g6)	{
	return	-40012576;
}

short int S4xNIJte0(int aPMQ9Tq,unsigned int i93Zg_nt,unsigned long MCN5NA,short r6b_krI0C) {
 return (short)-1627304417;
}

char    qqSO6OTKjL(unsigned short int    a0Hd,int    g4K42_x,short    Jep_D36)    {
    return    0;
}

