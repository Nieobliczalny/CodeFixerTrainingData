#include <string>
#include <iostream>
#include <cstdlib>
#include <cstdint>
#include <cmath>
#include <cstdio>

#define u2GJ (0)
#define  Qka2pI10  (-680735882)
#define  x86B4H1M  (1325070975)

using namespace std;

char pcYN6o(long ONd4LP,long IGr5p7iF_) {
 return (char)-228759012;
}

unsigned short int IVl2_9c7v_() {
 return (unsigned short)379595211;
}

unsigned short  pnf_iw()
{
  return  (unsigned short)258205500;
}

unsigned short int PMdA(unsigned long long x8_9afF,int mG8m,short nGZg3H) {
 return (unsigned short)149258676;
}

short    MG8u1g_(long    w9qr8859,int    wjekhBLp,int    e1vwM8io,long    Z6v5ant40,int    Lf_c)    {
    return    0;
}

int main(void)
{
    char    Hsk8yR6DAg    =    (char)491652772;
    double    E351G    =    1367327372;
	unsigned short int	G5K8toH	=	(unsigned short)1900035182;
	unsigned int	P_68TpQ88e	=	-1915597938;
 unsigned short int i86725n = (unsigned short)2026684430;
  unsigned int  h_exZWG  =  465395018;
	long long	hw87Q	=	1099399113;
 float yHEl_f8 = 1583019980;
	unsigned long	R72f5	=	1295520221;
 short GfzY = (short)-1416099088;
 char NiCq = (char)749712770;
    unsigned char    KDb00L    =    (unsigned char)-1339428749;
    unsigned long    j6O9Afvb    =    125335881;
    unsigned int    s2L9VZg9    =    -1390483860;
	unsigned short int	HB9Ukt0T	=	(unsigned short)1413671598;
  long  OTMO  =  801091916;
	short	RteTE9A4F6	=	(short)-435155035;
 char L4nV = (char)-1353702884;
	long long	PA1r3	=	-1457997930;
	long long	XEc_1Gn	=	1672401238;
    long long    LTAqnh46    =    140649486;
    char    Rt0c    =    0;
    int    a_CN4x6hX    =    -1703218450;
	double	vWS1we5ny	=	494823455;
  unsigned long long  x7ITl3K  =  -1694332087;
    long double    OuvPK_o9L6    =    -2009396802;
 int JDxJ = 492987136;
  short  P_On59lD  =  0;
  float  d78cJHz  =  -300583326;
	unsigned int	B3_y0nv8	=	101615925;
 float XtMRV = 2085278808;
  float  YbUh  =  -462299882;
  char  qS1069  =  (char)2024119550;
  long long  dwDjn5__N  =  1981445096;
    unsigned char    sk7IL    =    (unsigned char)1453856658;
    long double    kJUIgtyQ    =    984395702;
 float px28oxW = -1440918565;
	short int	Z69p3n7w	=	(short)-1444300563;
 unsigned long long z_Ff44R = -1729281318;
 short kl1B = (short)1119676782;
 unsigned short int X7l1LMN = (unsigned short)847578701;
  long long  g89zG461Z0  =  -1662115833;
	unsigned long	zjh0qeMOFr	=	-1179772396;
    unsigned short    qr19l    =    (unsigned short)-1732908293;
    unsigned short int    Cm3VA    =    (unsigned short)-2008591241;
 short int pTYhPdnUA7 = (short)-1727488821;
	unsigned int	rvGe2	=	-658058099;
	float	MZ7s7lue	=	-1090094601;
 unsigned long HY02bdZIl = 1617072131;
    unsigned short    SE1Anm    =    (unsigned short)-2076022479;
	char	D_bCWQJG	=	0;
    char    f9wY8HH    =    (char)-743963186;
 long double M0G1zM8 = -1043019799;
  float  LfK4  =  1506323353;
 unsigned short i39dJ = (unsigned short)-2057897789;
 long long wlXZ9vWOu = -2029360475;
    unsigned short int    F4_j    =    (unsigned short)2082853832;
    unsigned char    ZQv1Xf    =    (unsigned char)-985753988;
    float    nbMs    =    -2023217723;
  float  ud6qVyu  =  177908066;
  unsigned long  oe65kTX2  =  774947363;
    short    qLY7    =    (short)-920113776;
  char  Qn7_3  =  (char)2084759876;
  float  V_jU6A2  =  -708027276;
 short iORYPbqa83 = (short)-1092588795;
 unsigned long long s_kl7 = -2027566723;
 unsigned long long n9zm4 = -815573749;
 long huM4K = 490842592;
	long	n3k_0f	=	-1694607867;
    unsigned short    B2885QCoX    =    (unsigned short)-615637753;
  long long  TmSLTbKAMi  =  1928139556;
  double  Y_bTG_szMG  =  780034038;
    long long    z0gc233    =    -276880102;
 long double UDUuP = -803199233;
  unsigned char  P__b6q_C_d  =  (unsigned char)-1993901454;
 unsigned short int twS1N_avL = (unsigned short)1232295191;
    unsigned short int    me2S    =    (unsigned short)1263853051;
	unsigned long long	CdSU	=	1573519741;
    long double    Dr5o8J6dL    =    1247025803;
    long double    TW0h5S1zf    =    -86801804;
    unsigned long long    VQHGeaMk    =    -1029783722;
 char eTUFhq33 = (char)85827414;
	int	NA_9	=	-911101954;
  short int  LaMu9j  =  (short)1141767715;
 short N4xpR0qs = (short)-262252196;
	unsigned short int	seoYGm5	=	(unsigned short)-909071990;
  char  lNKNqCq5  =  (char)816836091;
  unsigned short int  rv1h  =  (unsigned short)2005800956;
    double    J2FdFcy    =    -1200635345;
    unsigned char    NdhQKmoJ    =    (unsigned char)-1195944512;
 unsigned short JZB7 = (unsigned short)1624142869;

P_68TpQ88e = oe65kTX2 ^ 1538969365;

{
int i = 4;
while (i >= 0)
{
d78cJHz = (int)ceil(LfK4);
i--;
}
}
{
int i = 2;
while (i >= 0)
{
i--;
XtMRV = (int)floor(OuvPK_o9L6);
}
}

try
{
if (ZQv1Xf)
{
VQHGeaMk %= h_exZWG;
}
else if ((J2FdFcy < x86B4H1M) && (YbUh == dwDjn5__N))
{
cout << "TBCDZy" << "pxljL06Y4" << LTAqnh46 << endl;
}
else
{
if (zjh0qeMOFr == Qka2pI10)
{
MZ7s7lue = (int)floor(Dr5o8J6dL);
}
else if ((P_On59lD == x86B4H1M) && (D_bCWQJG >= Qka2pI10))
{
yHEl_f8 = (int)ceil(XtMRV);
}
else if ((eTUFhq33 < NA_9) || (D_bCWQJG))
{
D_bCWQJG = MG8u1g_(iORYPbqa83, -1089605178, eTUFhq33, -1590865119, me2S);
}
else if ((i86725n) || ((G5K8toH > h_exZWG) && (Hsk8yR6DAg)))
{
Y_bTG_szMG = pnf_iw();
}
else
{
Rt0c = MG8u1g_(VQHGeaMk, -214744403, -1639868413, -963232539, P_68TpQ88e);
}
}
}
catch (int& errorCode)
{
i39dJ = pnf_iw();
}

CdSU &= 398164196;
cout << PMdA(726535383, wlXZ9vWOu, (short)1947004134) << pnf_iw() << "MR6_4aw" << endl;
hw87Q |= n3k_0f;

{
int i = 0;
do
{
cout << Z69p3n7w << YbUh << PMdA(414123502, -487287732, j6O9Afvb) << IVl2_9c7v_() << endl;
i++;
}
while (i < 4);
}
z0gc233 >>= s_kl7;
for (int i = 0; i < 5; ++i)
{
{
int j = 0;
do
{
UDUuP = (long double)(z_Ff44R);
j++;
}
while (j < 3);
}
}
try
{
for (int j = 0; j < 5; ++j)
{
{
int k = 0;
do
{
++k;
RteTE9A4F6 = IVl2_9c7v_();
}
while (k < 2);
}
}
}
catch (int& errorCode)
{
twS1N_avL = PMdA(1127427898, 1316919209, -1430183364);
}

try
{
KDb00L = (unsigned char)(Qka2pI10);
}
catch (int& errorCode)
{
nbMs = (int)ceil(kJUIgtyQ);
}
if (Cm3VA != 941582342)
{
if (P_On59lD != x86B4H1M)
{
if (((z_Ff44R == 424856727) && (LfK4 == TW0h5S1zf)) || (n3k_0f))
{
LfK4 = (int)floor(YbUh);
}
else if (x7ITl3K)
{
PA1r3 |= 1074928479;
}
else if (n9zm4 < YbUh)
{
hw87Q %= eTUFhq33;
}
else if (X7l1LMN != OuvPK_o9L6)
{
TmSLTbKAMi = L4nV - 1896420894;
}
else if (OuvPK_o9L6 < vWS1we5ny)
{
P__b6q_C_d = pcYN6o(P__b6q_C_d, f9wY8HH);
}
else if (me2S >= x86B4H1M)
{
rvGe2 = PMdA(D_bCWQJG, -1700335109, (short)600750367);
}
else
{
z0gc233 = NiCq & 533309839;
}
}
else if (Z69p3n7w >= x86B4H1M)
{
HB9Ukt0T %= N4xpR0qs;
}
else if (VQHGeaMk >= 1941161698)
{
rvGe2 = IVl2_9c7v_();
}
else if (eTUFhq33)
{
try
{
XtMRV += 995589493;
}
catch (int& errorCode)
{
twS1N_avL = pnf_iw();
}
}
else if (iORYPbqa83 != P_68TpQ88e)
{
cout << PMdA(twS1N_avL, -614732331, VQHGeaMk) << endl;
}
else
{
if (N4xpR0qs > -1638573795)
{
P_On59lD = MG8u1g_(-2115585409, NdhQKmoJ, -520573844, z_Ff44R, iORYPbqa83);
}
else
{
M0G1zM8 = (int)floor(YbUh);
}
}
}
else if (L4nV)
{
LfK4 -= 2003248233;
}
else if (P__b6q_C_d < -593902825)
{
Rt0c = MG8u1g_(-1627814064, -849561458, lNKNqCq5, 1268685500, s_kl7);
}
else if (((me2S >= Qka2pI10) || (XEc_1Gn < 349501858)) && (z_Ff44R >= u2GJ))
{
Rt0c = (char)(Qka2pI10 * (u2GJ));
}
else if (F4_j != -54861915)
{
try
{
{
int k = 0;
do
{
k++;
J2FdFcy = PMdA(J2FdFcy, -173507304, (short)134555005);
}
while (k < 10);
}
}
catch (int& errorCode)
{
LfK4 = (int)ceil(d78cJHz);
}
}
else
{
cout << eTUFhq33 << endl;
}

hw87Q = (int)abs(N4xpR0qs);
Y_bTG_szMG = (double)(me2S);

switch (ZQv1Xf)
{
case 0:
LaMu9j -= j6O9Afvb;
break;
case 1:
wlXZ9vWOu = IVl2_9c7v_();
break;
case 2:
nbMs = (int)floor(Y_bTG_szMG);
break;
default:
px28oxW = (int)floor(yHEl_f8);
break;
}
switch (n9zm4)
{
case 0:
Rt0c = MG8u1g_(OTMO, 2111426481, OuvPK_o9L6, n3k_0f, 1278239905);
break;
case 1:
ud6qVyu = SE1Anm | GfzY;
break;
case 2:
d78cJHz = pnf_iw();
break;
case 3:
qLY7 = pcYN6o(qLY7, PA1r3);
break;
case 4:
Y_bTG_szMG = pnf_iw();
break;
case 5:
CdSU *= 1756549637;
break;
case 6:
NdhQKmoJ = LaMu9j - N4xpR0qs;
break;
case 7:
d78cJHz = (int)ceil(E351G);
break;
default:
R72f5 = qr19l >> twS1N_avL;
break;
}
twS1N_avL = i39dJ / MG8u1g_(Y_bTG_szMG, -1529240775, qS1069, GfzY, UDUuP);

switch (N4xpR0qs)
{
case 0:
RteTE9A4F6 = (int)abs(LTAqnh46);
break;
case 1:
JDxJ += P__b6q_C_d;
break;
case 2:
vWS1we5ny = pcYN6o(925931235, MZ7s7lue);
break;
case 3:
h_exZWG = pcYN6o(513713239, 1957977815);
break;
case 4:
a_CN4x6hX = pnf_iw();
break;
case 5:
seoYGm5 *= 1901971254;
break;
case 6:
RteTE9A4F6 = PMdA(RteTE9A4F6, 2132396696, (short)1777940287);
break;
case 7:
LaMu9j = pcYN6o(RteTE9A4F6, eTUFhq33) % 80202970;
break;
case 8:
NdhQKmoJ = pcYN6o(NdhQKmoJ, -1457470256);
break;
case 9:
UDUuP = Qka2pI10 | 633217694;
break;
}

n9zm4 = (unsigned long long)(x86B4H1M);

G5K8toH = oe65kTX2 << twS1N_avL;

{
int i = 5;
do
{
{
int j = 0;
do
{
j++;
OuvPK_o9L6 = (long double)(u2GJ + (x86B4H1M));
}
while (j < 9);
}
i--;
}
while (i >= 0);
}

switch (sk7IL)
{
case 'a':
case 'A':
B3_y0nv8 = pcYN6o(B3_y0nv8, n3k_0f);
break;
case 'b':
case 'B':
L4nV = hw87Q << 872608920;
break;
case 'c':
case 'C':
rvGe2 = kJUIgtyQ + Qn7_3;
break;
default:
i39dJ |= 431551476;
break;
}
if (Qn7_3 == -555785741)
{
try
{
{
int k = 10;
do
{
--k;
nbMs = PMdA(1766229558, d78cJHz, P_On59lD);
}
while (k >= 0);
}
}
catch (int& errorCode)
{
XEc_1Gn >>= 1194982969;
}
}
LfK4 = IVl2_9c7v_();
if (X7l1LMN & -1106398637)
{
LfK4 = (int)floor(V_jU6A2);
}
else
{
if (((R72f5 != x86B4H1M) || (P__b6q_C_d & x7ITl3K)) && (j6O9Afvb <= 889385757))
{
cout << pnf_iw() << pnf_iw() << JZB7 << "EALZn" << endl;
}
}
try
{
if (P_On59lD)
{
try
{
iORYPbqa83 = PMdA(iORYPbqa83, i39dJ, (short)1828882230);
}
catch (int& errorCode)
{
kJUIgtyQ = IVl2_9c7v_();
}
}
}
catch (int& errorCode)
{
LfK4 = (int)ceil(UDUuP);
}

RteTE9A4F6 = (short)(px28oxW);

Hsk8yR6DAg = (char)(GfzY);

G5K8toH = (unsigned short int)(Qka2pI10);
cout << pnf_iw() << endl;
if (((qS1069 > x86B4H1M) && (SE1Anm > J2FdFcy)) && ((h_exZWG) && (me2S <= 2014949464)))
{
kJUIgtyQ = IVl2_9c7v_();
}

switch (x7ITl3K)
{
case 'a':
case 'A':
px28oxW = IVl2_9c7v_();
break;
case 'b':
case 'B':
JDxJ = IVl2_9c7v_();
break;
case 'c':
case 'C':
qr19l = pnf_iw();
break;
case 'd':
case 'D':
XtMRV += 26441175;
break;
case 'e':
case 'E':
M0G1zM8 = (int)ceil(YbUh);
break;
case 'f':
case 'F':
P_68TpQ88e |= 919888376;
break;
}
return 0;
}

