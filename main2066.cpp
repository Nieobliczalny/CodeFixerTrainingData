#include <cstdint>
#include <iostream>
#include <cstdio>
#include <cstdlib>
#include <string>
#include <cmath>

#define	vot_9	(0)


short	JsUY(long	n15Q4B1r)
{
	return	-1619450866;
}

long long	Zk8nV()	{
	return	-114305308;
}

long	Fk7Fsu(long	e2POwct,unsigned long	jq8Vy3IIa,char	zrwOjL9C,unsigned int	Ca0Fv)
{
	return	703956269;
}

int	rodP38vm(unsigned long	zorl,unsigned char	hsuQ,char	fK9gQ8F,short	IOWYUDeR7m)
{
	return	0;
}

int main(int argc, char** argv)
{
	float	f1K0R865,	fQN_Gc2lC,	ig2G_njk,	TB194xV7O,	LD8MD,	dwPs1H,	iVSbg3H,	uc21RJ,	Zz6l_,	frxE5,	Xc6y1C8C49,	WP2XjCXJX,	l0S_534Jc,	LVxg,	IkD6b0QD18,	VHCcNZR,	mJWY,	lwb6g5,	h9z91VgoAM,	JPKrMa,	wD11e5Fa2,	Rr1p_3U21,	GZKEb_,	vNl30cdt,	BFyb8h59M7,	w9zLR943,	k5s8CaA,	CrW3VHH,	TEkAZHb7,	zvId,	H9nOu,	GILbQz0Q1,	esB70gUl,	knCUW__,	QB9F,	yDPmi6E2KK,	sj5Uf,	Yoal8,	EMvNW_,	Eko0OP_UO,	wgvq,	AvqBWmCKWr,	XnhrLu3f8U,	DQ60Qfn,	jw4wSv2G,	W_koBvQ6,	x8P8Pt67,	GXV24GmmJ3,	L0_a_rAS,	W64062m5,	xgyU,	q9XPLHIeH,	yaG2g4b7H5,	VzwvPs2m7v,	BjMda2bo,	nHcnU,	eA_3P,	ok9nuh16L,	I_kEobC6nK,	fK2YBPO,	PxT0RDi1H,	sPJ4qF6f,	GMLo6M,	XVtr,	CA5Bx2,	j4y6,	hzW6es21,	MGnE18;
	f1K0R865	=	-1018853013;
	fQN_Gc2lC	=	-373763735;
	ig2G_njk	=	80025833;
	TB194xV7O	=	1651971917;
	LD8MD	=	-1783390396;
	dwPs1H	=	-738842471;
	iVSbg3H	=	1159810571;
	uc21RJ	=	1334681855;
	Zz6l_	=	-260052209;
	frxE5	=	2054003400;
	Xc6y1C8C49	=	-1995914981;
	WP2XjCXJX	=	2072675782;
	l0S_534Jc	=	2063855262;
	LVxg	=	192793961;
	IkD6b0QD18	=	673028635;
	VHCcNZR	=	-1107714334;
	mJWY	=	-2131771174;
	lwb6g5	=	1891992292;
	h9z91VgoAM	=	-1886078099;
	JPKrMa	=	-1871590857;
	wD11e5Fa2	=	181952767;
	Rr1p_3U21	=	846751541;
	GZKEb_	=	1576293834;
	vNl30cdt	=	61361336;
	BFyb8h59M7	=	1719716023;
	w9zLR943	=	-1106933886;
	k5s8CaA	=	1176287608;
	CrW3VHH	=	-501082565;
	TEkAZHb7	=	129472621;
	zvId	=	1259265217;
	H9nOu	=	778447997;
	GILbQz0Q1	=	-1791190198;
	esB70gUl	=	-612445802;
	knCUW__	=	-744334818;
	QB9F	=	-1428973777;
	yDPmi6E2KK	=	-424739753;
	sj5Uf	=	1050965753;
	Yoal8	=	437218955;
	EMvNW_	=	-223265193;
	Eko0OP_UO	=	1585001849;
	wgvq	=	-1978231879;
	AvqBWmCKWr	=	-1223226461;
	XnhrLu3f8U	=	607262542;
	DQ60Qfn	=	2130049504;
	jw4wSv2G	=	1480614724;
	W_koBvQ6	=	-1065785725;
	x8P8Pt67	=	-808469383;
	GXV24GmmJ3	=	-1852693625;
	L0_a_rAS	=	-897098007;
	W64062m5	=	-2065073391;
	xgyU	=	1557882532;
	q9XPLHIeH	=	-1117837781;
	yaG2g4b7H5	=	479354730;
	VzwvPs2m7v	=	-1769769219;
	BjMda2bo	=	467375753;
	nHcnU	=	1458940615;
	eA_3P	=	791211152;
	ok9nuh16L	=	-1378663738;
	I_kEobC6nK	=	1431966298;
	fK2YBPO	=	695839846;
	PxT0RDi1H	=	1296214997;
	sPJ4qF6f	=	-936054455;
	GMLo6M	=	981663948;
	XVtr	=	780480506;
	CA5Bx2	=	-1602211012;
	j4y6	=	231894368;
	hzW6es21	=	261326610;
	MGnE18	=	43446840;
  unsigned long  S6A9f,  R5R4yhdd,  TR4K1,  oIJ2_9,  O_wmr_001;
  S6A9f  =  0;
  R5R4yhdd  =  0;
  TR4K1  =  0;
  oIJ2_9  =  0;
  O_wmr_001  =  0;

std::cout << Zk8nV() << "yYN7N2I" << I_kEobC6nK << std::endl;

if (((dwPs1H >= vot_9) && (L0_a_rAS != vot_9)) && (knCUW__))
{
hzW6es21 += 1182333851;
}
else
{
vNl30cdt = fK2YBPO / CrW3VHH;
}

switch (oIJ2_9)
{
case 0:
IkD6b0QD18 = (int)abs(uc21RJ);
break;
case 1:
VHCcNZR = Zk8nV();
break;
case 2:
BjMda2bo = Fk7Fsu(BjMda2bo, -1859604161, VHCcNZR, 247363627);
break;
case 3:
q9XPLHIeH = (R5R4yhdd > -693808730) ? wgvq : PxT0RDi1H;
break;
default:
h9z91VgoAM = JsUY(-13444396);
break;
}
{
int i = 0;
while (i < 10)
{
ok9nuh16L = (float)(esB70gUl);
++i;
}
}
switch (R5R4yhdd)
{
case 'a':
case 'A':
TR4K1 = rodP38vm(CrW3VHH, EMvNW_, ig2G_njk, 1908074754);
break;
case 'b':
case 'B':
Zz6l_ -= LVxg;
break;
case 'c':
case 'C':
fK2YBPO = (int)ceil(QB9F);
break;
default:
ig2G_njk = Zk8nV();
break;
}
if (jw4wSv2G)
{
hzW6es21 = (float)(vot_9 + (-1810582323));
}
else
{
iVSbg3H /= 1472489217;
}

try
{
try
{
std::cout << "RL_C" << "K07w77HLa" << "BIUZB" << fK2YBPO << std::endl;
}
catch (int& errorCode)
{
VHCcNZR /= VzwvPs2m7v;
}
}
catch (int& errorCode)
{
DQ60Qfn = Zk8nV();
}
std::cout << wD11e5Fa2 << Zk8nV() << Zk8nV() << std::endl;

{
int i = 0;
do
{
for (int j = 0; j < 5; j++)
{
{
int k = 5;
do
{
--k;
TEkAZHb7 = (int)ceil(h9z91VgoAM);
}
while (k >= 0);
}
}
++i;
}
while (i < 5);
}
if ((Rr1p_3U21 / rodP38vm(-1557010764, frxE5, zvId, 1283221360)) > 1822948060)
{
PxT0RDi1H = XVtr / rodP38vm(-527768707, -748584763, 1190725150, sj5Uf);
}
return 0;
}

