#include <cstdio>
#include <cmath>
#include <cstdint>
#include <iostream>
#include <cstdlib>
#include <string>

#define	kevG	(-477742275)
#define	qYh18CYg	(-1735290069)
#define	dyB8HF0L	(0)

using namespace std;

int main(int argc, char** argv) {
 unsigned short int Ot2u751x = (unsigned short)-286413433;
 int o2wr = -851309;
  long  DkN5yr_  =  1629785529;
 char uQSm4bjz = (char)-1346029452;
 unsigned long p4o5 = -210079000;
	long long	MHOJlAG0	=	439351570;
	short	EuM5_Cc	=	(short)573738881;
	unsigned int	xl59pT	=	-1460292050;
  long double  zGdw  =  -1727593654;
    long long    cbcN4CO_    =    -598362757;
    unsigned short    J0T3aR    =    (unsigned short)556941920;
  char  ymAg6n  =  (char)-1389664033;
  unsigned short int  FU1h69cV6  =  (unsigned short)-672189473;
    int    F5IR62    =    -979113929;
 long long R_4p_ = 1392611738;
 short O3m5X98r9 = (short)2010164049;
    unsigned int    bEN0Hbd0M    =    530732882;
  unsigned long long  DA6xM  =  -2016910069;
 long of1HDF68 = -1344436060;
	double	M1TUm1WCI	=	-955330444;
	short	JnYm7	=	(short)1787368314;
	char	MUaq	=	(char)-1177370440;
  short  SihB_bn_Y  =  (short)1764031261;
 long double Sx8a0 = -1937796162;
	int	YSdao3	=	-394131941;
	unsigned char	zTp0R49c	=	(unsigned char)756903834;
    short    a0Vi    =    (short)778393328;
 char EDE739Am = (char)-1226420370;
    unsigned long long    pT5__T    =    -1964365272;
 unsigned int XVvQ4LTk9u = -206602718;
    unsigned long    X2svj30g1    =    0;
	unsigned char	sXy5Xs	=	(unsigned char)1179564132;
 unsigned long nsFk = 1260756252;
	unsigned long long	nc8b7q	=	1985373565;
 long long y3C3J = 1023309048;
  long double  aaf6V6VEY1  =  -1191110906;
 unsigned long long g_kH = -17453414;
  unsigned long  NLz7dHZY1  =  -84162738;
    long double    j93V8    =    -1252918251;
	unsigned short	c4T9sm	=	(unsigned short)1290470271;
    long long    up0o__NB    =    -1476349890;
  short  zxpt7_7m  =  0;
  unsigned long  e8Xm67x  =  1496424466;
    unsigned long long    TjoKQcT4xj    =    -817647748;
 short int PuJ4y8U8at = (short)959259014;
 long rA2DDap = 941889968;
    unsigned short    h_272S4v    =    (unsigned short)-1642549992;
  unsigned long long  n5uKs87_  =  912359589;
 unsigned char l0Ew5Xykpn = 0;
 int MuIP7e = 0;
    long double    XxOjzi    =    -403979217;
  int  n9f6xQR7  =  -78709761;
    long double    Plg0_M5    =    -539600017;
    unsigned char    n6ihO7tsA2    =    0;
	long double	GI650d909	=	-1277171712;
	unsigned long	CJfgp1wl	=	733055830;
	long	LOMmh5g5f	=	-1239076531;
 float jHQX4jCWw2 = -1006256805;
  float  p_bCU4908  =  -744746010;
    unsigned long    PHd6KqZ2g    =    1371882362;
  unsigned short  W7DcIG  =  (unsigned short)-989605236;
    unsigned short    o4Ho4d    =    (unsigned short)2116485958;
    short int    Es3f5WtM    =    (short)1027316884;
	short int	QedS7TF	=	(short)-727387469;
  double  VdQ8O_2fG  =  1298380266;
  unsigned long long  fgeWv  =  -1384133535;
    unsigned int    QDX7aH    =    739241415;
	unsigned int	kS9KQ9h	=	589130620;
    short int    GaO2rj7VG    =    (short)496512011;
  long double  Gc0YX  =  295979984;
	unsigned char	M2DA9248	=	(unsigned char)-663818294;
 unsigned long dzuA = 847236188;
    long    owZ393    =    -1760462487;
    long    So_kt    =    1934574874;
	long	ByP98H78	=	976715829;
    unsigned int    N8j2d    =    -1376991173;
  short int  MP9b3RCmR  =  (short)-220278403;
 short int Pfe_9X = (short)-1704416258;
    short int    aj1f984HR1    =    (short)2116261518;
  short  YpISkK  =  (short)-1807277235;
 long double fD9hh251q = -1130737846;
	long long	drCF	=	308844944;
    long    x9x_    =    708087455;
    unsigned long long    Ww7uj2HB4H    =    1328379350;
    int    E9kl1c99i    =    -486944746;
	unsigned long	p25n0	=	-1592362934;
    float    Xg4K    =    1118344455;
    unsigned short int    LNslsNDZ    =    (unsigned short)1090126394;
    long    Kxy5    =    376377967;
  double  Mqpk48_He5  =  973772365;
  char  d9Ee  =  (char)-1558274513;
    short int    bcMsQ6q    =    (short)1464765752;
    unsigned char    KDeh    =    (unsigned char)-579536923;

if (c4T9sm != -281556279)
{
try
{
{
int k = 0;
while (k < 2)
{
++k;
MP9b3RCmR ^= N8j2d;
}
}
}
catch (int& errorCode)
{
Gc0YX = (int)ceil(VdQ8O_2fG);
}
}
else if ((J0T3aR <= -1362767279) || (GaO2rj7VG & kevG))
{
EuM5_Cc = (short)(dyB8HF0L + (qYh18CYg));
}
else if (EuM5_Cc <= 1862395953)
{
switch (n9f6xQR7)
{
case 0:
n9f6xQR7 <<= 717253426;
break;
case 1:
LNslsNDZ %= 431599012;
break;
case 2:
p_bCU4908 = (int)ceil(Gc0YX);
break;
case 3:
Kxy5 -= 899121052;
break;
case 4:
g_kH++;
break;
case 5:
ymAg6n ^= 909793775;
break;
case 6:
dzuA = (PuJ4y8U8at >= Sx8a0) ? SihB_bn_Y : -1578049554;
break;
case 7:
h_272S4v = ymAg6n / YpISkK;
break;
default:
nsFk = a0Vi % YSdao3;
break;
}
}
else if (Plg0_M5 != YpISkK)
{
try
{
owZ393 += pT5__T;
}
catch (int& errorCode)
{
Sx8a0 = (int)ceil(p_bCU4908);
}
}
else if (M2DA9248 == qYh18CYg)
{
{
int j = 0;
while (j < 9)
{
j++;
if (((QedS7TF == kevG) && (W7DcIG & kevG)) || (rA2DDap != bcMsQ6q))
{
o4Ho4d = rA2DDap | YSdao3;
}
}
}
}
else
{
{
int j = 0;
while (j < 4)
{
j++;
{
int k = 10;
while (k >= 0)
{
--k;
YSdao3++;
}
}
}
}
}
Ww7uj2HB4H = (unsigned long long)(FU1h69cV6);
F5IR62 = 572910979 / MuIP7e;

{
int i = 0;
while (i < 8)
{
try
{
if (Plg0_M5 == zxpt7_7m)
{
E9kl1c99i = owZ393 << xl59pT;
}
else if (nc8b7q != kevG)
{
j93V8 = (int)floor(Gc0YX);
}
else
{
n9f6xQR7 = l0Ew5Xykpn >> xl59pT;
}
}
catch (int& errorCode)
{
M1TUm1WCI = (int)floor(p_bCU4908);
}
++i;
}
}
for (int i = 0; i < 6; i++)
{
std::cout << "QX73" << "x_ieH" << rA2DDap << Sx8a0 << std::endl;
}

owZ393 = (long)(DkN5yr_);

MP9b3RCmR = (int)abs(bcMsQ6q);

if ((XxOjzi) || (LNslsNDZ))
{
{
int j = 4;
do
{
--j;
{
int k = 3;
do
{
EuM5_Cc = (fD9hh251q) ? Xg4K : EDE739Am;
k--;
}
while (k >= 0);
}
}
while (j >= 0);
}
}

for (int i = 7; i >= 0; i--)
{
std::cout << "EN0eDV" << "utzt60pv" << LNslsNDZ << nsFk << std::endl;
}

if (x9x_ >= dyB8HF0L)
{
switch (owZ393)
{
case 0:
CJfgp1wl ^= 1837796450;
break;
case 1:
ByP98H78 >>= 388588817;
break;
case 2:
p_bCU4908 = (int)floor(Plg0_M5);
break;
case 3:
uQSm4bjz >>= bcMsQ6q;
break;
case 4:
XxOjzi = (int)floor(aaf6V6VEY1);
break;
case 5:
uQSm4bjz = MuIP7e - 454388439;
break;
case 6:
xl59pT >>= PuJ4y8U8at;
break;
case 7:
Es3f5WtM = (int)abs(GI650d909);
break;
}
}
else if (M2DA9248 & MHOJlAG0)
{
XxOjzi = (int)ceil(jHQX4jCWw2);
}
else if ((n6ihO7tsA2 != -681288582) && (bcMsQ6q < Pfe_9X))
{
if (bcMsQ6q > 1281414347)
{
{
int k = 9;
while (k >= 0)
{
QedS7TF &= 708662782;
--k;
}
}
}
else
{
zxpt7_7m = (short)(a0Vi * (dyB8HF0L));
}
}
else if (((NLz7dHZY1 == dyB8HF0L) || (l0Ew5Xykpn > -814690241)) || (d9Ee > fgeWv))
{
if (uQSm4bjz > GaO2rj7VG)
{
for (int k = 4; k >= 0; k--)
{
aaf6V6VEY1 = (int)abs(n9f6xQR7);
}
}
else if (bcMsQ6q <= n9f6xQR7)
{
DA6xM = d9Ee % kS9KQ9h;
}
else
{
switch (nsFk)
{
case 0:
Es3f5WtM = dzuA ^ 398080310;
break;
case 1:
ByP98H78 = (int)abs(drCF);
break;
default:
p_bCU4908 = (int)floor(zGdw);
break;
}
}
}
else if ((MP9b3RCmR > dyB8HF0L) || (rA2DDap != d9Ee))
{
if (xl59pT & MP9b3RCmR)
{
{
int k = 8;
do
{
--k;
x9x_ *= 1181172518;
}
while (k >= 0);
}
}
}
else
{
Mqpk48_He5 = (double)(dyB8HF0L + (kevG));
}
for (int i = 3; i >= 0; --i)
{
if (bEN0Hbd0M & -1598623639)
{
XVvQ4LTk9u++;
}
}

F5IR62 = (int)abs(rA2DDap);
return 0;
}

