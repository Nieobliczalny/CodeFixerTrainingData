#include <cmath>
#include <string>
#include <cstdio>
#include <cstdlib>
#include <cstdint>
#include <iostream>



unsigned short int    R4i1g_1hH(unsigned char    LhptHX292z)
{
    return    (unsigned short)-1900557132;
}

short int	E2I2f7wJ(short int	Z_r4,char	hr29QVF7T)	{
	return	0;
}

long	A09l2N1hlY(char	fBM_,unsigned long long	c40RvAV,long long	r8sZ)	{
	return	1822451555;
}

int main(void) {
    unsigned long long    SP3y5X99y7;
    SP3y5X99y7    =    842347365;
  double  k_k_PxQ79n;
  k_k_PxQ79n  =  -2126151548;
 char aop_8yMy7;
 aop_8yMy7 = (char)282222376;
	float	C95p526i;
	C95p526i	=	806445480;
    int    H_A7iQN;
    H_A7iQN    =    559047650;
  char  eQB1P49Oyh;
  eQB1P49Oyh  =  (char)-745755090;
 unsigned long long Zf5D_;
 Zf5D_ = -1899500700;
    short    KA_9U;
    KA_9U    =    (short)560657129;
    int    M3Bpg8_fCT;
    M3Bpg8_fCT    =    324109641;
	long	Ilh4;
	Ilh4	=	598357206;
    short    w1jI;
    w1jI    =    (short)-1595055482;
    long    U0mP2Z5kZP;
    U0mP2Z5kZP    =    118610830;
	unsigned long long	QM_mb5E5;
	QM_mb5E5	=	1659530051;
  unsigned long  n8rE;
  n8rE  =  958694402;
  unsigned short int  B4uFu3V;
  B4uFu3V  =  (unsigned short)1773226735;
    unsigned int    eC0UA9e;
    eC0UA9e    =    -554577061;
  unsigned long long  Twq5M6y;
  Twq5M6y  =  -530578081;
	long long	oCU9Z;
	oCU9Z	=	2083026841;
    int    NUSre;
    NUSre    =    -138819256;
 unsigned short rgx2;
 rgx2 = (unsigned short)470311396;
	unsigned short	u8s9j;
	u8s9j	=	(unsigned short)-1503996349;
    unsigned short    V4k3a78X;
    V4k3a78X    =    (unsigned short)143396086;
	unsigned short	Mc4q_soZ9;
	Mc4q_soZ9	=	1575424560;
	unsigned char	DhpiHhaG;
	DhpiHhaG	=	-689066875;
	unsigned char	V9kOyhWW;
	V9kOyhWW	=	(unsigned char)-1900490990;
  unsigned char  F31SGZUm;
  F31SGZUm  =  (unsigned char)-375477275;
    unsigned long    yGILdL;
    yGILdL    =    -1424590527;
 long double T2a0s;
 T2a0s = -938436343;
  short int  OePT8nzL9g;
  OePT8nzL9g  =  (short)1873484256;
    long long    cKTxI68;
    cKTxI68    =    -34138366;
	unsigned int	vxOzdd4X;
	vxOzdd4X	=	-148341118;
 char NETK8k;
 NETK8k = (char)-1924992023;
  short  kKLh_Oh;
  kKLh_Oh  =  (short)1033725983;
 double D0_2g;
 D0_2g = -1233160021;
    short int    Pyh8;
    Pyh8    =    (short)-949155779;
	unsigned int	xipe3M_T;
	xipe3M_T	=	396496314;
  long  vLDP;
  vLDP  =  -32845825;
	unsigned short int	Er8xN2x;
	Er8xN2x	=	(unsigned short)-152402195;
  short  Xl_cT;
  Xl_cT  =  (short)-1320269040;
  short int  Hkqb3165;
  Hkqb3165  =  (short)893609939;
  unsigned char  Usatuzix;
  Usatuzix  =  0;
 long rbyk1MHq0T;
 rbyk1MHq0T = 70923519;
 float QxE00;
 QxE00 = -1102368333;
  unsigned short int  e5rUX;
  e5rUX  =  (unsigned short)-1243443380;
  long double  l3rr;
  l3rr  =  1586697548;
	unsigned long	MMCN;
	MMCN	=	-1372917755;
    short    Kx_11;
    Kx_11    =    (short)518116409;
 long long e6Hhdj0;
 e6Hhdj0 = -268746531;
    float    Qz35;
    Qz35    =    -1307178965;
	short	jTF42gji59;
	jTF42gji59	=	(short)1773363148;
 short int TXY7k81;
 TXY7k81 = (short)-1936317525;
  unsigned long long  yGx2;
  yGx2  =  1205091374;
	unsigned short	K_g2VYWl;
	K_g2VYWl	=	(unsigned short)-1869966269;
    unsigned long long    NghtH_5;
    NghtH_5    =    534027533;
	short int	IAg3j4;
	IAg3j4	=	(short)53597043;
    unsigned short    EJgHutjW5H;
    EJgHutjW5H    =    (unsigned short)-1840345559;
 double BOZ469z;
 BOZ469z = -386461642;
	float	slj_;
	slj_	=	-659815763;
	char	d2qL0nz_L9;
	d2qL0nz_L9	=	(char)1801429424;
    int    A9yxF30;
    A9yxF30    =    -1813028963;
 long long cXP7_5_o9l;
 cXP7_5_o9l = -934301697;
    int    LcB5vhQc;
    LcB5vhQc    =    -1976344258;
 float B7UE6EY66L;
 B7UE6EY66L = 302160138;
 unsigned short Ks42;
 Ks42 = (unsigned short)1031302179;
    unsigned int    ZsB0eMh3Yw;
    ZsB0eMh3Yw    =    -628901963;
    long    o_ZO09NKK;
    o_ZO09NKK    =    606566482;
 float mMPIph4_;
 mMPIph4_ = 1183576683;
  float  HJWt5;
  HJWt5  =  1478892279;
 short int jilhFo;
 jilhFo = (short)-1989225204;
    short    dcNLf5;
    dcNLf5    =    (short)-1197882604;
	long	sn3We5kGqN;
	sn3We5kGqN	=	-1233866818;
  char  NC4U_;
  NC4U_  =  (char)1673542654;
    unsigned char    OTy67nk56Q;
    OTy67nk56Q    =    2065793875;
  char  OIhGW3aG2f;
  OIhGW3aG2f  =  (char)-2076578193;
 long JgrDJcT;
 JgrDJcT = -1745132017;
	short	h4Y714O;
	h4Y714O	=	(short)770119385;
	long	u2SOyCXyB;
	u2SOyCXyB	=	-1799168991;
    unsigned short int    sJ3r0g3FBf;
    sJ3r0g3FBf    =    (unsigned short)-986237664;
	short int	xKz1HO;
	xKz1HO	=	(short)1850415216;
  long double  xp14H7600;
  xp14H7600  =  -746818777;
	unsigned int	Y8ao40pU1a;
	Y8ao40pU1a	=	-593024656;
	unsigned char	O6Hn2j80;
	O6Hn2j80	=	(unsigned char)881255971;
  unsigned int  GLOSR_2V_;
  GLOSR_2V_  =  1195941492;
 char APGw2qH;
 APGw2qH = (char)1671860360;
	unsigned short int	be9gP9CoSD;
	be9gP9CoSD	=	(unsigned short)-443089883;
    long double    PJ896;
    PJ896    =    -1529859887;
  unsigned long  oKE76kB7_3;
  oKE76kB7_3  =  274441989;
	unsigned short int	lRHjE37t;
	lRHjE37t	=	(unsigned short)239762194;
  unsigned long  NQ_X;
  NQ_X  =  552748074;
	double	GN62WhH43;
	GN62WhH43	=	-2070043913;
 long long cnYFPj;
 cnYFPj = 0;
	unsigned short int	Xi3pnjR5x;
	Xi3pnjR5x	=	(unsigned short)1964966451;
  int  uq5BZ2m;
  uq5BZ2m  =  1420180745;
 char irSP9Q2;
 irSP9Q2 = (char)-469088597;
  double  JN7zF;
  JN7zF  =  -2039987393;
	unsigned long	NZHBvZ;
	NZHBvZ	=	-213016938;
	unsigned char	FUtn3bl9D;
	FUtn3bl9D	=	(unsigned char)1267767308;
	long	Ly1x5_5;
	Ly1x5_5	=	1909781417;
  int  t61Hq_1U;
  t61Hq_1U  =  393514478;
 short int Gd86d;
 Gd86d = (short)1514650817;
    short    At4rHL8;
    At4rHL8    =    (short)-2012577793;
  short  dB70gAY13;
  dB70gAY13  =  (short)1393410309;

if (yGILdL > 2026971407)
{
TXY7k81 = A09l2N1hlY(TXY7k81, FUtn3bl9D, DhpiHhaG);
}
else
{
Kx_11 = R4i1g_1hH((unsigned char)-1787507524);
}

try
{
{
int j = 8;
while (j >= 0)
{
Usatuzix = E2I2f7wJ(rgx2, Y8ao40pU1a);
j--;
}
}
}
catch (int& errorCode)
{
U0mP2Z5kZP--;
}

std::cout << yGILdL << A09l2N1hlY((char)-1968861511, Y8ao40pU1a, -310772323) << "DExL" << std::endl;

{
int i = 3;
while (i >= 0)
{
i--;
rbyk1MHq0T = (int)abs(Qz35);
}
}

{
int i = 0;
while (i < 9)
{
++i;
for (int j = 0; j < 7; ++j)
{
Usatuzix = E2I2f7wJ((short)159327632, (char)1760248057);
}
}
}

std::cout << R4i1g_1hH((unsigned char)-1726691236) << V4k3a78X << "wj8xCG6t" << E2I2f7wJ(-1173760921, 336921344) << std::endl;

C95p526i = (float)(aop_8yMy7);
switch (IAg3j4)
{
case 'a':
case 'A':
n8rE = R4i1g_1hH((unsigned char)-520644400);
break;
case 'b':
case 'B':
Y8ao40pU1a = R4i1g_1hH((unsigned char)1157748203);
break;
default:
HJWt5 = (int)floor(k_k_PxQ79n);
break;
}

if ((M3Bpg8_fCT != 964141263) && ((cKTxI68 < OIhGW3aG2f) || (OIhGW3aG2f == -681104957)))
{
try
{
if (be9gP9CoSD == -1656540544)
{
OTy67nk56Q = R4i1g_1hH(OTy67nk56Q);
}
}
catch (int& errorCode)
{
k_k_PxQ79n = (int)floor(C95p526i);
}
}
O6Hn2j80 = FUtn3bl9D / Usatuzix;

for (int i = 2; i >= 0; i--)
{
JN7zF = (int)floor(slj_);
}
for (int i = 0; i < 1; i++)
{
for (int j = 0; j < 7; ++j)
{
Qz35 = A09l2N1hlY(Qz35, 592743501, QM_mb5E5);
}
}
{
int i = 8;
while (i >= 0)
{
--i;
if (oKE76kB7_3 <= 216417929)
{
B7UE6EY66L = (int)floor(l3rr);
}
}
}
cKTxI68 -= Mc4q_soZ9;

if (At4rHL8 > yGx2)
{
{
int j = 0;
while (j < 7)
{
u8s9j = (unsigned short)(Mc4q_soZ9);
j++;
}
}
}
std::cout << "ZH6b" << R4i1g_1hH((unsigned char)-1380142637) << E2I2f7wJ(Qz35, Twq5M6y) << "g4IU9w" << Twq5M6y << std::endl;
if (slj_ > B7UE6EY66L)
{
try
{
if (o_ZO09NKK > -266801015)
{
w1jI = (Er8xN2x) ? rgx2 : e5rUX;
}
else
{
xipe3M_T >>= dcNLf5;
}
}
catch (int& errorCode)
{
QxE00 = A09l2N1hlY(-1913936323, rbyk1MHq0T, -1397800035);
}
}

k_k_PxQ79n = (int)ceil(D0_2g);

if (((F31SGZUm <= 815190368) || (SP3y5X99y7 != -1509334010)) && (sJ3r0g3FBf <= 768271823))
{
if (yGx2 == 535857362)
{
{
int k = 0;
do
{
HJWt5 = (int)abs(Pyh8);
k++;
}
while (k < 8);
}
}
}

if (((Mc4q_soZ9 <= 172699978) || (k_k_PxQ79n != -2112567610)) || (NETK8k > -1911627780))
{
V9kOyhWW = (unsigned char)(QM_mb5E5);
}

std::cout << E2I2f7wJ(QxE00, (char)948285281) << JgrDJcT << std::endl;

if (APGw2qH <= 598651264)
{
for (int j = 3; j >= 0; j--)
{
switch (Twq5M6y)
{
case 'a':
uq5BZ2m = R4i1g_1hH((unsigned char)826528740) ^ eC0UA9e;
break;
case 'b':
mMPIph4_ = A09l2N1hlY((char)235150635, PJ896, OTy67nk56Q);
break;
}
}
}
{
int i = 0;
while (i < 1)
{
++i;
{
int j = 0;
do
{
{
int k = 5;
do
{
jilhFo = R4i1g_1hH(jilhFo);
k--;
}
while (k >= 0);
}
++j;
}
while (j < 8);
}
}
}

return 0;
}

