#include <cmath>
#include <iostream>
#include <cstdio>
#include <cstdint>
#include <cstdlib>
#include <string>


using std::cout;
using std::endl;
using std::abs;
using std::ceil;
using std::floor;

int main(void)
{
    unsigned char    Z6O7;
    Z6O7    =    (unsigned char)438142559;
 unsigned short int K_9ne8L_;
 K_9ne8L_ = (unsigned short)-1388881635;
    short    Ffhlq;
    Ffhlq    =    (short)134971177;
  unsigned long  E1d_AD2vU1;
  E1d_AD2vU1  =  -39328420;
    short int    BtV2J;
    BtV2J    =    0;
	unsigned long	zo1kCc1;
	zo1kCc1	=	-1916186118;
  long double  NZ_R5w8;
  NZ_R5w8  =  -1726982608;
	short	QB_vD_D36U;
	QB_vD_D36U	=	(short)-1026641463;
  int  yAwzm69;
  yAwzm69  =  2109082950;
  unsigned short int  q9Iuvg69Y;
  q9Iuvg69Y  =  (unsigned short)1829456179;
 long OKl1KolL8R;
 OKl1KolL8R = -1697099834;
	long	T5e36mQS5;
	T5e36mQS5	=	310633968;
  double  oZW6A;
  oZW6A  =  -84418757;
  short  fTHiqb5r;
  fTHiqb5r  =  (short)-839482640;
 short QlcA0;
 QlcA0 = (short)250877717;
	long	U86Z;
	U86Z	=	2108733423;
	unsigned short int	yGlQu7Q;
	yGlQu7Q	=	(unsigned short)-46077291;
 int SgeEE;
 SgeEE = -267683700;
  unsigned char  evaeT614u;
  evaeT614u  =  (unsigned char)1559930191;
    unsigned char    TuO7t3;
    TuO7t3    =    0;
  long  yBEO6;
  yBEO6  =  -2025377977;
    long    ioB5B3WU;
    ioB5B3WU    =    -1949201922;
	unsigned long	gq0pC7Fn_Y;
	gq0pC7Fn_Y	=	0;
	int	xrAv;
	xrAv	=	-483181870;
	float	m3ZM;
	m3ZM	=	-1755234763;
    unsigned char    aZ08;
    aZ08    =    (unsigned char)1924126776;
  unsigned short  zbEJ73IgFM;
  zbEJ73IgFM  =  (unsigned short)2136811153;
 short h9gbSDj;
 h9gbSDj = (short)1373747826;
  long long  tZFJ;
  tZFJ  =  2105247525;
  unsigned long  hccz;
  hccz  =  1287276521;
  char  t9zW;
  t9zW  =  (char)643202414;
	int	Uhbz;
	Uhbz	=	774794430;
	char	z40r_1pvE;
	z40r_1pvE	=	(char)1254338388;
	char	R9i72KtEzR;
	R9i72KtEzR	=	(char)-1786930522;
  long  qgc3;
  qgc3  =  1633174747;
	unsigned long long	NLzT1hUumM;
	NLzT1hUumM	=	-1534642070;
    char    RwUuXpljQ;
    RwUuXpljQ    =    (char)-681404367;
  unsigned char  X93yWj9Jc;
  X93yWj9Jc  =  (unsigned char)1826726110;
 unsigned short PieMo3xp4;
 PieMo3xp4 = 0;
  long long  quxMYc;
  quxMYc  =  418002979;
	unsigned char	TP620Q_;
	TP620Q_	=	(unsigned char)2026325825;
  unsigned int  Fzse6;
  Fzse6  =  766848714;
    unsigned short    KawN;
    KawN    =    (unsigned short)1906123654;
	unsigned long long	a9S4;
	a9S4	=	-1591302960;
  short  tWTaOy;
  tWTaOy  =  (short)-760109416;
	short int	Nm_Bvk79;
	Nm_Bvk79	=	(short)-1714549960;
  unsigned long  SP3MecbO;
  SP3MecbO  =  1689010017;
  long double  zS0kYu;
  zS0kYu  =  -1701078689;
    float    YVyRX;
    YVyRX    =    1195538854;
 short fr607e_i2;
 fr607e_i2 = (short)106970965;
    unsigned long    q040;
    q040    =    -1530860825;
  long long  hoY9_;
  hoY9_  =  -168486527;
	unsigned char	nKU61FI;
	nKU61FI	=	(unsigned char)1767780339;
  unsigned char  Y65db2qu;
  Y65db2qu  =  (unsigned char)10783312;
  unsigned long  m_mDw7Ch;
  m_mDw7Ch  =  947125497;
  unsigned char  vgN034Y;
  vgN034Y  =  (unsigned char)651326244;
 long double wX3oykG3L;
 wX3oykG3L = -1700537533;
	short	gU3e4wA;
	gU3e4wA	=	(short)-1282797025;
  int  lv50j_W;
  lv50j_W  =  553744073;

std::cout << tZFJ << T5e36mQS5 << "vTmpScCF" << std::endl;
q040 = -640047990 / gq0pC7Fn_Y;
if (QlcA0 < 835751451)
{
{
int j = 10;
while (j >= 0)
{
j--;
fr607e_i2 = (short)(m_mDw7Ch);
}
}
}
else
{
{
int j = 2;
while (j >= 0)
{
--j;
hoY9_ = (long long)(a9S4);
}
}
}

{
int i = 0;
do
{
++i;
{
int j = 0;
do
{
quxMYc -= 1556351038;
++j;
}
while (j < 4);
}
}
while (i < 10);
}

std::cout << wX3oykG3L << std::endl;
try
{
z40r_1pvE = (char)(lv50j_W);
}
catch (int& errorCode)
{
a9S4--;
}
if (Nm_Bvk79 == -1428553525)
{
Ffhlq = X93yWj9Jc | 1517478969;
}
wX3oykG3L = (int)floor(zS0kYu);
std::cout << Ffhlq << "Ql_l" << Uhbz << PieMo3xp4 << a9S4 << std::endl;

{
int i = 6;
do
{
yGlQu7Q >>= 1898529653;
i--;
}
while (i >= 0);
}
std::cout << YVyRX << std::endl;

NZ_R5w8 -= 36722585;

U86Z >>= evaeT614u;
h9gbSDj = (short)(PieMo3xp4 + (1529692383));

{
int i = 0;
while (i < 5)
{
h9gbSDj = (int)abs(RwUuXpljQ);
++i;
}
}
Uhbz = q9Iuvg69Y * Fzse6;
fr607e_i2 -= zo1kCc1;

oZW6A = (int)floor(YVyRX);

return 0;
}

