#include <string>
#include <cstdint>
#include <iostream>
#include <cstdio>
#include <cmath>
#include <cstdlib>

#define  Di4w_3o4  (1488858925)
#define  Z3IBB9P6a  (596502026)
#define	S1WU_	(1975403267)
#define    q6TuJJc0V    (-231756636)
#define    Cio72    (0)

using namespace std;

int main(void) {
    unsigned short    XxoRWA,    TjSPtoG206,    GD4x,    w33L6_5G,    J0RRY,    iE8mSUw6X,    g0cA06I50,    E6d4n0OXvX,    pORZ,    ATi3bQv4,    WN_Ymu,    W78bWOYGHq,    DKSu9y5Vo,    igcE13xc,    f4Hhv1474j,    a6e43TZ_NL,    pri5q91_1,    a4_6DjhMc,    ISSQ,    Bhzg9RGcP,    SrjpS,    I5nSYiF_,    Gh_50g,    Pj_Zln643,    pV3WDvPU,    wvQDZ619O8,    jAGh87vUY,    Xc0avT5,    NbUC94,    iy71a,    zj1g,    iW996RYwvp,    Q9BP92A,    D6e2,    K7cmFe1F,    Q0vbYg08h,    p3r0_kQ01,    flHGHl6,    SjL0,    h0O1IZ548h,    ww_WD0b7N5,    P473erPXN8,    t73wy9,    kys8Y8,    i3U_dwx6,    PE5E7C,    qbyD,    XXh7z52,    T81S,    Ava265s,    L9dbUw,    A_7hckab,    TB7g,    v_g9_,    FA2Hw58N,    I9Kk7Xk8,    r8w_JqB,    ERiVTgN50q,    M9pUd9Ioz,    U1BB,    QQQ9a17yQk,    iO7qma3,    JcTw_qawz,    Uy332YLgz0,    wK67fn8k7M,    PV2Gn,    G4n5YY,    D6W3GFe9l,    v0eUG,    G7Hc2l_WI0,    FRZufb6X4O,    R56u5hZ3V1,    KQucSDGn,    FzfW6MB,    tBIq5ce,    ql2if2BxS,    rxNFy9248T,    i3R3Qi5O,    zVwYv51O;
        XxoRWA    =    TjSPtoG206    =    GD4x    =    w33L6_5G    =    J0RRY    =    iE8mSUw6X    =    g0cA06I50    =    E6d4n0OXvX    =    pORZ    =    ATi3bQv4    =    WN_Ymu    =    W78bWOYGHq    =    DKSu9y5Vo    =    igcE13xc    =    f4Hhv1474j    =    a6e43TZ_NL    =    pri5q91_1    =    a4_6DjhMc    =    ISSQ    =    Bhzg9RGcP    =    SrjpS    =    I5nSYiF_    =    Gh_50g    =    Pj_Zln643    =    pV3WDvPU    =    wvQDZ619O8    =    jAGh87vUY    =    Xc0avT5    =    NbUC94    =    iy71a    =    zj1g    =    iW996RYwvp    =    Q9BP92A    =    D6e2    =    K7cmFe1F    =    Q0vbYg08h    =    p3r0_kQ01    =    flHGHl6    =    SjL0    =    h0O1IZ548h    =    ww_WD0b7N5    =    P473erPXN8    =    t73wy9    =    kys8Y8    =    i3U_dwx6    =    PE5E7C    =    qbyD    =    XXh7z52    =    T81S    =    Ava265s    =    L9dbUw    =    A_7hckab    =    TB7g    =    v_g9_    =    FA2Hw58N    =    I9Kk7Xk8    =    r8w_JqB    =    ERiVTgN50q    =    M9pUd9Ioz    =    U1BB    =    QQQ9a17yQk    =    iO7qma3    =    JcTw_qawz    =    Uy332YLgz0    =    wK67fn8k7M    =    PV2Gn    =    G4n5YY    =    D6W3GFe9l    =    v0eUG    =    G7Hc2l_WI0    =    FRZufb6X4O    =    R56u5hZ3V1    =    KQucSDGn    =    FzfW6MB    =    tBIq5ce    =    ql2if2BxS    =    rxNFy9248T    =    i3R3Qi5O    =    zVwYv51O    =    (unsigned short)-1068163322;
	short	HD2avv,	f7nVS;
		HD2avv	=	f7nVS	=	0;

i3U_dwx6 = (unsigned short)(Z3IBB9P6a);

std::cout << "Q27nA93Bg" << std::endl;
{
int i = 0;
while (i < 1)
{
i++;
i3R3Qi5O /= 750591949;
}
}
ATi3bQv4--;

if (ATi3bQv4 > -1148767922)
{
{
int j = 2;
while (j >= 0)
{
--j;
if (pV3WDvPU >= -1549892759)
{
pri5q91_1 = SjL0 / p3r0_kQ01;
}
else if (iW996RYwvp == i3R3Qi5O)
{
HD2avv = (int)abs(f7nVS);
}
else if (A_7hckab < Di4w_3o4)
{
HD2avv = (int)abs(f7nVS);
}
else if (GD4x == DKSu9y5Vo)
{
iy71a--;
}
else if ((KQucSDGn >= zVwYv51O) && (v_g9_ > Di4w_3o4))
{
v0eUG = XXh7z52 % 1029446899;
}
else if (FzfW6MB)
{
L9dbUw = Q0vbYg08h + J0RRY;
}
else
{
iE8mSUw6X = D6e2 % f4Hhv1474j;
}
}
}
}

std::cout << "YZuS61jL" << JcTw_qawz << wvQDZ619O8 << "mD0pjYO" << "a7S97zbS" << std::endl;
std::cout << "g008" << w33L6_5G << J0RRY << Uy332YLgz0 << rxNFy9248T << std::endl;

std::cout << iy71a << std::endl;
if (R56u5hZ3V1 < 1578035020)
{
std::cout << DKSu9y5Vo << JcTw_qawz << "ybAqxx" << "p23282yJ" << "HtO7ACN5j7" << std::endl;
}
else
{
switch (ISSQ)
{
case 'a':
case 'A':
Ava265s = ERiVTgN50q - 2007775868;
break;
case 'b':
case 'B':
kys8Y8 = Cio72 / i3U_dwx6;
break;
case 'c':
case 'C':
pri5q91_1 |= 61670753;
break;
case 'd':
case 'D':
J0RRY = FRZufb6X4O - iy71a;
break;
case 'e':
case 'E':
Xc0avT5 = PE5E7C >> 516806560;
break;
case 'f':
case 'F':
ISSQ = (G7Hc2l_WI0 == 1479019220) ? FzfW6MB : (unsigned short)S1WU_;
break;
case 'g':
case 'G':
f7nVS = (int)abs(HD2avv);
break;
default:
f7nVS = (int)abs(HD2avv);
break;
}
}

for (int i = 0; i < 6; ++i)
{
switch (D6W3GFe9l)
{
case 0:
pV3WDvPU <<= a4_6DjhMc;
break;
case 1:
iW996RYwvp = TB7g - 876070464;
break;
}
}

switch (QQQ9a17yQk)
{
case 0:
Xc0avT5 /= SrjpS;
break;
default:
i3U_dwx6 = pri5q91_1 % ISSQ;
break;
}

NbUC94 = (unsigned short)(Cio72 + (238694847));

try
{
switch (NbUC94)
{
case 'a':
case 'A':
zVwYv51O -= 384355450;
break;
case 'b':
case 'B':
kys8Y8 &= ISSQ;
break;
case 'c':
case 'C':
pri5q91_1 *= 1926861240;
break;
case 'd':
case 'D':
a4_6DjhMc = XXh7z52 % 195703918;
break;
case 'e':
case 'E':
h0O1IZ548h = FA2Hw58N ^ 1996777049;
break;
case 'f':
case 'F':
NbUC94 = (Bhzg9RGcP <= pri5q91_1) ? (unsigned short)Z3IBB9P6a : (unsigned short)Z3IBB9P6a;
break;
default:
ERiVTgN50q++;
break;
}
}
catch (int& errorCode)
{
T81S = q6TuJJc0V ^ 2096157116;
}
std::cout << "LtV1Aq" << P473erPXN8 << "poyzI1tf" << std::endl;
if (Uy332YLgz0)
{
switch (M9pUd9Ioz)
{
case 0:
FA2Hw58N *= XXh7z52;
break;
case 1:
ww_WD0b7N5 = ERiVTgN50q << 79079125;
break;
case 2:
G7Hc2l_WI0 >>= R56u5hZ3V1;
break;
default:
XxoRWA = (h0O1IZ548h <= -11538372) ? ATi3bQv4 : (unsigned short)-527927187;
break;
}
}
else
{
try
{
switch (U1BB)
{
case 0:
flHGHl6 = ATi3bQv4 * SrjpS;
break;
case 1:
w33L6_5G %= iE8mSUw6X;
break;
case 2:
TjSPtoG206 = (pV3WDvPU & S1WU_) ? wK67fn8k7M : jAGh87vUY;
break;
case 3:
HD2avv = (int)abs(f7nVS);
break;
case 4:
HD2avv = (int)abs(f7nVS);
break;
case 5:
f7nVS = (int)abs(HD2avv);
break;
case 6:
rxNFy9248T = Bhzg9RGcP * KQucSDGn;
break;
case 7:
r8w_JqB /= ql2if2BxS;
break;
default:
f7nVS = (int)abs(HD2avv);
break;
}
}
catch (int& errorCode)
{
Xc0avT5 = FA2Hw58N & T81S;
}
}
zVwYv51O++;
f4Hhv1474j = iE8mSUw6X & FA2Hw58N;
kys8Y8 >>= P473erPXN8;
if ((ql2if2BxS & Di4w_3o4) && (TjSPtoG206 != -38648515))
{
XxoRWA &= D6W3GFe9l;
}
else
{
f7nVS = (int)abs(HD2avv);
}
try
{
Gh_50g ^= FRZufb6X4O;
}
catch (int& errorCode)
{
f7nVS = (int)abs(HD2avv);
}

std::cout << M9pUd9Ioz / HD2avv << std::endl;
if ((G4n5YY < XxoRWA) || (f7nVS >= -406279796))
{
ISSQ++;
}
else if (XxoRWA > Cio72)
{
for (int j = 0; j < 2; ++j)
{
iO7qma3++;
}
}
else if (f7nVS == 525821032)
{
try
{
if (iO7qma3 == QQQ9a17yQk)
{
qbyD++;
}
else if (SrjpS != P473erPXN8)
{
R56u5hZ3V1 = a6e43TZ_NL & QQQ9a17yQk;
}
else
{
A_7hckab = g0cA06I50 | 1296559672;
}
}
catch (int& errorCode)
{
zj1g = E6d4n0OXvX * 1128805382;
}
}
else if ((tBIq5ce < h0O1IZ548h) || ((iW996RYwvp & S1WU_) && (zj1g == -429139808)))
{
r8w_JqB = (unsigned short)(Cio72 + (-1662093369));
}
else if ((kys8Y8 <= SjL0) && ((Uy332YLgz0 != zj1g) || (v_g9_)))
{
if ((J0RRY <= S1WU_) || (v0eUG >= zj1g))
{
if (tBIq5ce <= Cio72)
{
J0RRY %= iO7qma3;
}
}
else if (a6e43TZ_NL < jAGh87vUY)
{
{
int k = 8;
do
{
v_g9_ %= 2043640632;
k--;
}
while (k >= 0);
}
}
else if (f7nVS < PE5E7C)
{
if ((f4Hhv1474j <= q6TuJJc0V) || (NbUC94 == 1217389763))
{
iO7qma3 <<= h0O1IZ548h;
}
}
else
{
if (XxoRWA >= 256520025)
{
pri5q91_1--;
}
else
{
v0eUG = DKSu9y5Vo % i3U_dwx6;
}
}
}
else if (iO7qma3 >= WN_Ymu)
{
HD2avv = (int)abs(f7nVS);
}
else
{
switch (XxoRWA)
{
case 0:
iO7qma3 = zj1g * 2060780655;
break;
case 1:
Q9BP92A += 1035600318;
break;
case 2:
WN_Ymu ^= DKSu9y5Vo;
break;
}
}

NbUC94 -= L9dbUw;

{
int i = 0;
while (i < 10)
{
ql2if2BxS *= QQQ9a17yQk;
i++;
}
}
if (PV2Gn != i3R3Qi5O)
{
HD2avv = (int)abs(f7nVS);
}
for (int i = 1; i >= 0; i--)
{
for (int j = 4; j >= 0; j--)
{
f7nVS = (int)abs(HD2avv);
}
}

if ((I9Kk7Xk8 & 1949109752) && ((WN_Ymu == A_7hckab) && (p3r0_kQ01 != ISSQ)))
{
try
{
HD2avv = (int)abs(f7nVS);
}
catch (int& errorCode)
{
jAGh87vUY /= 314930120;
}
}

for (int i = 5; i >= 0; i--)
{
std::cout << "mjgbn" << std::endl;
}
{
int i = 2;
while (i >= 0)
{
if (f4Hhv1474j)
{
{
int k = 4;
do
{
HD2avv = (int)abs(f7nVS);
k--;
}
while (k >= 0);
}
}
else
{
if (iW996RYwvp <= Z3IBB9P6a)
{
ATi3bQv4 %= 435280518;
}
else if (pORZ & -1273407886)
{
f7nVS = (int)abs(HD2avv);
}
else if (zj1g)
{
A_7hckab >>= 2124619524;
}
else if (Gh_50g >= -818514763)
{
zj1g = r8w_JqB - 832059338;
}
else
{
a6e43TZ_NL = (Ava265s > Di4w_3o4) ? (unsigned short)-1557255942 : (unsigned short)-114805207;
}
}
--i;
}
}
{
int i = 8;
do
{
i--;
switch (P473erPXN8)
{
case 'a':
R56u5hZ3V1 /= FRZufb6X4O;
break;
case 'b':
G4n5YY &= 1341780386;
break;
default:
FRZufb6X4O = rxNFy9248T % 1941833499;
break;
}
}
while (i >= 0);
}
{
int i = 0;
do
{
{
int j = 2;
do
{
PV2Gn *= I5nSYiF_;
--j;
}
while (j >= 0);
}
++i;
}
while (i < 8);
}
FA2Hw58N = (unsigned short)(ERiVTgN50q);

{
int i = 3;
do
{
D6e2 = (unsigned short)(ISSQ);
i--;
}
while (i >= 0);
}
a4_6DjhMc = kys8Y8 + 1637097451;

f7nVS = (int)abs(HD2avv);
return 0;
}

