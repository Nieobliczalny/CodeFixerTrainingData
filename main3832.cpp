#include <cstdlib>
#include <cstdint>
#include <cmath>
#include <iostream>
#include <string>
#include <cstdio>


using std::cout;
using std::endl;
using std::abs;
using std::ceil;
using std::floor;

int main() {
    double    bVIeqer;
    bVIeqer    =    -709489073;
  long double  OG_Ybml;
  OG_Ybml  =  -3406521;
  long  YU74u5;
  YU74u5  =  -945097864;
	short int	WR4b4;
	WR4b4	=	(short)-58508833;
	unsigned long	T4Apdl;
	T4Apdl	=	1427525562;
    char    gUTn1SYNr;
    gUTn1SYNr    =    (char)695264478;
	unsigned long	KGy355L8;
	KGy355L8	=	-574252908;
 unsigned long long M78MDR4124;
 M78MDR4124 = 188424676;
 unsigned short int r_0_6D812K;
 r_0_6D812K = (unsigned short)20490580;
	unsigned long	rq9j8b;
	rq9j8b	=	-1402426924;
    unsigned char    a__9Pti;
    a__9Pti    =    (unsigned char)-273931128;
    unsigned char    i8Y9;
    i8Y9    =    (unsigned char)-35263573;
    unsigned char    dJ7ntbI;
    dJ7ntbI    =    -447830801;
    int    M_2I;
    M_2I    =    1938432061;
  float  uh3iR;
  uh3iR  =  2017093715;
 unsigned short uGp3Z2;
 uGp3Z2 = (unsigned short)-1099907646;
	long	qrNh7c;
	qrNh7c	=	-1604981578;
 long x85K;
 x85K = -574448310;
	unsigned char	O2mH2_k;
	O2mH2_k	=	(unsigned char)785962917;
	unsigned int	R5q5gGiK99;
	R5q5gGiK99	=	574234526;
    float    QeB27382;
    QeB27382    =    -621854465;
 unsigned short int A88d8Y_7;
 A88d8Y_7 = 0;
 unsigned short Alr5h;
 Alr5h = (unsigned short)1286516598;
 unsigned short int u8lWB4S;
 u8lWB4S = (unsigned short)-945187161;
 float Xe6k_4l1s;
 Xe6k_4l1s = 785655937;
	short	WJvb0Mi8;
	WJvb0Mi8	=	(short)-1658111897;
 float zxZ8DPr;
 zxZ8DPr = -245890767;
	unsigned long long	g6hzCFU;
	g6hzCFU	=	-2044102019;
    unsigned char    vEg_t3j_;
    vEg_t3j_    =    (unsigned char)1447405944;
  unsigned short int  oUg_EQg;
  oUg_EQg  =  (unsigned short)-1965619367;
  int  s2Hjz;
  s2Hjz  =  2078647895;
	char	Y60FS;
	Y60FS	=	(char)792755939;
    unsigned long long    Bv7O1ekS;
    Bv7O1ekS    =    1115298790;
    unsigned long long    SBJ9We1;
    SBJ9We1    =    -2065525877;
  unsigned char  u_AV5IT;
  u_AV5IT  =  (unsigned char)1026800417;
  long double  YBL00iOl40;
  YBL00iOl40  =  1972751961;
 unsigned short int w282bR9;
 w282bR9 = (unsigned short)809291979;
  unsigned short  hlJ_KV3l;
  hlJ_KV3l  =  (unsigned short)-82826329;
  float  W3Ly;
  W3Ly  =  1833931234;
 double Ou3m;
 Ou3m = -629860775;
	double	ODXJ3p;
	ODXJ3p	=	-1430907842;
  char  PE84F5Q9b4;
  PE84F5Q9b4  =  (char)-1824134226;
    double    B1_Tnh_zX;
    B1_Tnh_zX    =    -562514751;
	float	Jbwf899;
	Jbwf899	=	-1281633584;
  char  TC304;
  TC304  =  0;
    unsigned long    Pu7B3YPGx;
    Pu7B3YPGx    =    2093372036;
    int    Zk7Y_;
    Zk7Y_    =    1470502373;
 unsigned long qi3oTVp98;
 qi3oTVp98 = -1381509382;
  short  O05K7F2e;
  O05K7F2e  =  (short)-479431611;
    short    hD2kb6Vuud;
    hD2kb6Vuud    =    0;
	char	z5luhm9;
	z5luhm9	=	0;
 unsigned short efNxJlj;
 efNxJlj = 0;
	double	E_2o1;
	E_2o1	=	326981915;
  int  WguG;
  WguG  =  680343563;
	long double	UyxV;
	UyxV	=	-1270115372;
    char    PJndbqnr;
    PJndbqnr    =    (char)1848430912;
 double r6zc_zc;
 r6zc_zc = -1832507288;
    char    kMOG25;
    kMOG25    =    (char)-1920429678;
    short int    nkmAKO;
    nkmAKO    =    (short)1763307991;
    long double    WLAsWIoxBW;
    WLAsWIoxBW    =    -69655466;
  long long  zm62th3;
  zm62th3  =  1686756109;
  unsigned long  d2e9Zp7V0;
  d2e9Zp7V0  =  -596842894;
    long double    j387YNo1i;
    j387YNo1i    =    -1595628584;

ODXJ3p = u_AV5IT | 1229955167;

TC304 = (int)abs(YU74u5) * 0;
if ((efNxJlj == oUg_EQg) || ((zxZ8DPr < 2129673829) || (dJ7ntbI == 10419166)))
{
i8Y9 = (unsigned char)(z5luhm9 + (2073693178));
}
else if (UyxV == 195861016)
{
w282bR9 = (unsigned short int)(WJvb0Mi8);
}
else if (Xe6k_4l1s)
{
uGp3Z2 >>= Zk7Y_;
}
else if (PJndbqnr > YBL00iOl40)
{
Xe6k_4l1s = (int)abs(W3Ly);
}
else
{
for (int j = 8; j >= 0; j--)
{
cout << "WG7kt" << i8Y9 << "J5krnH" << vEg_t3j_ << M_2I << endl;
}
}
{
int i = 7;
do
{
if (O2mH2_k != -1881848371)
{
PE84F5Q9b4 = (char)(WJvb0Mi8);
}
else if (dJ7ntbI >= -634021463)
{
{
int k = 0;
do
{
++k;
WR4b4 >>= 1289341942;
}
while (k < 5);
}
}
else if ((T4Apdl < vEg_t3j_) && (M_2I & 1632292006))
{
hlJ_KV3l &= 18744402;
}
else if (j387YNo1i != u8lWB4S)
{
for (int k = 0; k < 7; ++k)
{
QeB27382 = (int)floor(E_2o1);
}
}
else if ((KGy355L8 >= R5q5gGiK99) && (Alr5h <= kMOG25))
{
{
int k = 0;
while (k < 10)
{
++k;
WLAsWIoxBW = (int)ceil(zxZ8DPr);
}
}
}
else
{
if (r6zc_zc)
{
x85K = (int)abs(s2Hjz);
}
}
--i;
}
while (i >= 0);
}
Ou3m = (double)(UyxV);
WR4b4 = PJndbqnr | 224166270;
{
int i = 6;
do
{
switch (Zk7Y_)
{
case 'a':
j387YNo1i /= ODXJ3p;
break;
default:
WR4b4 = Pu7B3YPGx - 1247877342;
break;
}
--i;
}
while (i >= 0);
}
T4Apdl = UyxV / efNxJlj;

Ou3m = (double)(YU74u5);
switch (SBJ9We1)
{
case 'a':
case 'A':
zm62th3--;
break;
case 'b':
case 'B':
WLAsWIoxBW = (QeB27382 != -259056677) ? j387YNo1i : 781351111;
break;
default:
Pu7B3YPGx <<= Bv7O1ekS;
break;
}
cout << rq9j8b << endl;
a__9Pti *= T4Apdl;

zxZ8DPr = (int)ceil(Ou3m);
{
int i = 7;
do
{
--i;
{
int j = 1;
while (j >= 0)
{
Jbwf899 = (int)floor(QeB27382);
--j;
}
}
}
while (i >= 0);
}
return 0;
}

