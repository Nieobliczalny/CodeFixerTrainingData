#include <cstdio>
#include <iostream>
#include <cstdint>
#include <string>
#include <cmath>
#include <cstdlib>


using namespace std;

int main(int argc, char** argv)
{
    short    dj7R_HDs    =    (short)1709754527;
 long long aHtEMf = 0;
  double  ZURzs  =  -1781446428;
  int  d7G7LPn_9  =  1240240376;
  unsigned short int  vdEJ_2_  =  (unsigned short)1764972489;
    unsigned int    XUVO    =    -1081819566;
  unsigned short  U4Mfk  =  (unsigned short)-1115634184;
  unsigned int  ckv5l4p2lE  =  307734767;
 unsigned char J__U = (unsigned char)-463432372;
	double	f9KUBIdd	=	1394480482;
 short S6rFRrinr3 = (short)1830721925;
 short int KuDb1yChrH = (short)-183527460;
	short int	rH2RZ	=	-1835780294;
	int	fiWx	=	1860980120;
    unsigned long long    mVJQUR    =    1729345632;
    unsigned int    H3hF0i_j1g    =    1004933932;
	float	OJRHk45T	=	1506708446;
	char	B_TMz2kK	=	(char)-1499222617;
  unsigned short  UM5vi0  =  (unsigned short)1659224332;
    unsigned short int    Qwu7j    =    (unsigned short)2112681322;
    long long    A0Kix    =    2031443576;
  char  dm55  =  (char)1187127160;
 float p6bh0u809 = -1126368041;
	long	BWZX	=	-887893707;
	unsigned long	qEY_2y	=	1412019586;
  unsigned long  fEnZwOmOC  =  -271059377;
    long    BcMkC8E    =    1496023096;
  char  YmjUhR0  =  (char)-97396086;
    long    LRSp48B    =    -303651095;
 short md3hG = (short)758393925;
  short  V_V_h4A  =  (short)-844605767;
 unsigned int CaieD = 1109984632;
	short int	aA7q	=	0;
	unsigned short int	IU1U	=	(unsigned short)1742737485;
 short o7WII5D = (short)1038056904;
  double  m_54  =  1663964822;
    unsigned long long    C95Cgr    =    814468050;
    double    WRoyIw4    =    -1670803699;
 unsigned int rDkjK = -1784325499;
 unsigned short int N_wH1kqg = (unsigned short)1034259568;
	unsigned long	cUV4AHwx	=	-138607760;
	short	Uj_3zVp	=	(short)1699685794;
	unsigned long	A9ZZm0	=	0;
  float  jT_Gk80  =  -1812570846;
	short	XFkx0mp	=	(short)-1174130777;
	unsigned long long	r1PwM2	=	1963125224;
  long  url71  =  773147246;
 short int sDB2Ka2 = (short)-1643735183;
	char	X18_	=	(char)-1188113941;
 unsigned char F80dh7KEfm = (unsigned char)513825822;
	long long	MD_tn	=	881433242;
    long    ltq4BSW    =    -1091963272;
 unsigned long long yHA35z = 1410060482;
 unsigned short rtR_1mYr01 = (unsigned short)-924339121;
  short int  uh31c  =  (short)-1597785434;
 unsigned short int Z1hPPbh8 = 0;

ZURzs = (int)floor(OJRHk45T);
for (int i = 0; i < 6; ++i)
{
if (A0Kix == 1844255470)
{
XUVO = B_TMz2kK - mVJQUR;
}
else
{
for (int k = 0; k < 5; k++)
{
X18_ ^= 1997084053;
}
}
}

for (int i = 0; i < 7; ++i)
{
fiWx--;
}

try
{
switch (XUVO)
{
case 'a':
B_TMz2kK = XFkx0mp | 1768963336;
break;
case 'b':
WRoyIw4 = (int)ceil(ZURzs);
break;
case 'c':
C95Cgr = UM5vi0 / 1942654311;
break;
case 'd':
ckv5l4p2lE = fiWx ^ 170289984;
break;
case 'e':
r1PwM2 <<= XFkx0mp;
break;
case 'f':
ZURzs = (int)floor(f9KUBIdd);
break;
case 'g':
CaieD = rH2RZ - 1519668204;
break;
case 'h':
H3hF0i_j1g = vdEJ_2_ - 102720137;
break;
case 'i':
rtR_1mYr01 = A0Kix % IU1U;
break;
case 'j':
o7WII5D = o7WII5D << rDkjK;
break;
default:
dm55 = (int)abs(url71);
break;
}
}
catch (int& errorCode)
{
KuDb1yChrH = (int)abs(sDB2Ka2);
}
std::cout << -648376847 / aA7q << std::endl;

switch (d7G7LPn_9)
{
case 0:
uh31c -= KuDb1yChrH;
break;
default:
yHA35z = LRSp48B + F80dh7KEfm;
break;
}

try
{
S6rFRrinr3 *= 1286911870;
}
catch (int& errorCode)
{
m_54 = (int)floor(OJRHk45T);
}
std::cout << cUV4AHwx << r1PwM2 << BWZX << std::endl;
if (aHtEMf != 1157236448)
{
{
int j = 0;
while (j < 8)
{
dm55 <<= 1019431977;
++j;
}
}
}
else if ((cUV4AHwx) && (J__U < 829123733))
{
{
int j = 9;
while (j >= 0)
{
--j;
m_54 = (int)floor(jT_Gk80);
}
}
}
else if (C95Cgr >= -1582793563)
{
X18_ += Uj_3zVp;
}
else if (rH2RZ >= 1715753989)
{
{
int j = 0;
do
{
{
int k = 6;
do
{
C95Cgr += LRSp48B;
--k;
}
while (k >= 0);
}
++j;
}
while (j < 5);
}
}
else if ((aA7q) && (sDB2Ka2 < BWZX))
{
std::cout << dm55 << ltq4BSW << Qwu7j << std::endl;
}
else
{
std::cout << XUVO << std::endl;
}
try
{
if (f9KUBIdd > 1919900898)
{
rH2RZ /= MD_tn;
}
}
catch (int& errorCode)
{
WRoyIw4 = sDB2Ka2 / 627290615;
}

for (int i = 0; i < 1; ++i)
{
if (jT_Gk80)
{
{
int k = 9;
do
{
--k;
f9KUBIdd = (int)ceil(OJRHk45T);
}
while (k >= 0);
}
}
else
{
cUV4AHwx = sDB2Ka2 | 2060866883;
}
}
if (yHA35z <= C95Cgr)
{
for (int j = 8; j >= 0; --j)
{
if (mVJQUR == F80dh7KEfm)
{
Qwu7j = F80dh7KEfm << BWZX;
}
}
}
if (WRoyIw4 >= -176673349)
{
qEY_2y++;
}
else
{
for (int j = 0; j < 1; j++)
{
std::cout << "uG9c89" << "t1Qs" << yHA35z << "V0vQEhO" << rDkjK << std::endl;
}
}
switch (yHA35z)
{
case 0:
N_wH1kqg = dm55 % C95Cgr;
break;
case 1:
ZURzs = (int)floor(WRoyIw4);
break;
case 2:
BWZX = N_wH1kqg % 673784936;
break;
case 3:
ZURzs = (int)floor(OJRHk45T);
break;
case 4:
uh31c -= Uj_3zVp;
break;
case 5:
U4Mfk ^= 1162602519;
break;
case 6:
S6rFRrinr3 = C95Cgr + 251596827;
break;
}
MD_tn |= 1550541761;

switch (rtR_1mYr01)
{
case 'a':
WRoyIw4 = (int)ceil(jT_Gk80);
break;
case 'b':
H3hF0i_j1g >>= 448663578;
break;
case 'c':
url71 = (md3hG) ? -495888399 : 755793339;
break;
case 'd':
rtR_1mYr01 = fEnZwOmOC + f9KUBIdd;
break;
case 'e':
OJRHk45T = (int)floor(ZURzs);
break;
default:
Qwu7j = (d7G7LPn_9 != 1868676353) ? (unsigned short)-1219095149 : (unsigned short)-284059282;
break;
}
OJRHk45T = (int)floor(jT_Gk80);

if (X18_ < YmjUhR0)
{
C95Cgr *= 1115273827;
}
else
{
switch (YmjUhR0)
{
case 'a':
case 'A':
d7G7LPn_9 = KuDb1yChrH ^ YmjUhR0;
break;
case 'b':
case 'B':
OJRHk45T = (int)ceil(WRoyIw4);
break;
case 'c':
case 'C':
dm55 = (int)abs(ZURzs);
break;
case 'd':
case 'D':
XFkx0mp = (int)abs(aHtEMf) + (1730174298);
break;
case 'e':
case 'E':
A0Kix = p6bh0u809 * XUVO;
break;
case 'f':
case 'F':
ZURzs = (int)floor(WRoyIw4);
break;
}
}

try
{
for (int j = 2; j >= 0; j--)
{
{
int k = 0;
do
{
Uj_3zVp = H3hF0i_j1g * 2054760138;
++k;
}
while (k < 6);
}
}
}
catch (int& errorCode)
{
XFkx0mp = (int)abs(KuDb1yChrH);
}
switch (A0Kix)
{
case 'a':
case 'A':
qEY_2y = Z1hPPbh8 / 911559771;
break;
case 'b':
case 'B':
r1PwM2 = aA7q ^ XFkx0mp;
break;
case 'c':
case 'C':
rDkjK = fEnZwOmOC - N_wH1kqg;
break;
case 'd':
case 'D':
V_V_h4A *= 2123400344;
break;
case 'e':
case 'E':
ckv5l4p2lE = sDB2Ka2 - 961926573;
break;
case 'f':
case 'F':
J__U = BWZX / 1181235762;
break;
case 'g':
case 'G':
S6rFRrinr3 = N_wH1kqg & rDkjK;
break;
case 'h':
case 'H':
mVJQUR = Qwu7j - N_wH1kqg;
break;
}

return 0;
}

